...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ordervalue')">
                <el-table-column show-overflow-tooltip :prop="'ordervalue'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.ordervalue')" :width="100"  :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('orgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'orgsectorname'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.orgsectorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.orgcode')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('shortname')">
                <el-table-column show-overflow-tooltip :prop="'shortname'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.shortname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgname')">
                <el-table-column show-overflow-tooltip :prop="'orgname'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.orgname')" :width="380"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('ormxmglname')">
                <el-table-column show-overflow-tooltip :prop="'ormxmglname'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.ormxmglname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='ormxmgl' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'ormxmglredirect-view', height: 0,width: 0,title: $t('entities.ormxmgl.views.redirectview.title'),placement: '', isRedirectView: true,deResParameters: [
            ]
            ,parameters: [
            { pathName: 'ormxmgls', parameterName: 'ormxmgl' },
            { pathName: 'redirectview', parameterName: 'redirectview' }
            ]}" valueitem="ormxmglid">
                            <span>{{row.ormxmglname}}</span>
                        </app-column-link >
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('xmbh')">
                <el-table-column show-overflow-tooltip :prop="'xmbh'" :label="$t('entities.ormorgsector.xmbchoice_grid.columns.xmbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.xmbh}}</span>
                    </template>
                </el-table-column>
...
import XMBChoiceService from './xmbchoice-grid-service';
...
export default class XMBChoiceBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {XMBChoiceService}
     * @memberof XMBChoice
     */
    public service: XMBChoiceService = new XMBChoiceService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ordervalue',
            label: '排序',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.ordervalue',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgsectorname',
            label: '项目部名称',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.orgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '项目部编号',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'shortname',
            label: '项目部简称',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.shortname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgname',
            label: '所属组织',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.orgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormxmglname',
            label: 'ERP项目',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.ormxmglname',
            show: true,
            util: 'PX'
        },
        {
            name: 'xmbh',
            label: '项目编号',
            langtag: 'entities.ormorgsector.xmbchoice_grid.columns.xmbh',
            show: true,
            util: 'px'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORPXMBPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORPXMBPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormorgsector: keys.join(';') }),Object.assign({ ormorgsector: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormorgsector_xmbchoice_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormorgsector_xmbchoice_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORPXMBPickupGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORPXMBPickupGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
