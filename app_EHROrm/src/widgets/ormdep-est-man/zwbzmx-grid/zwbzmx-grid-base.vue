...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormdutyname')">
                <el-table-column show-overflow-tooltip :prop="'ormdutyname'" :label="$t('entities.ormdepestman.zwbzmx_grid.columns.ormdutyname')" :width="300"  :align="'left'" :sortable="'custom'">
              ...
              @formitemvaluechange="($event)=>{onGridItemValueChange(row,$event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                              <app-column-link deKeyField='ormduty' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'ormdutyredirect-view', height: 0,width: 0,title: $t('entities.ormduty.views.redirectview.title'),placement: '', isRedirectView: true,deResParameters: [
            ]
            ,parameters: [
            ...
            <template v-if="getColumnState('numcompilation')">
                <el-table-column show-overflow-tooltip :prop="'numcompilation'" :label="$t('entities.ormdepestman.zwbzmx_grid.columns.numcompilation')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='numcompilation' editorType="TEXTBOX" :value="row.numcompilation"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.ormdepestman.zwbzmx_grid.columns.createman')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.ormdepestman.zwbzmx_grid.columns.createdate')" :width="140"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
import ZWBZMXService from './zwbzmx-grid-service';
...
export default class ZWBZMXBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormdutyname',
            label: '职务',
            langtag: 'entities.ormdepestman.zwbzmx_grid.columns.ormdutyname',
            show: true,
            util: 'PX'
        },
        {
            name: 'numcompilation',
            label: '人数编制',
            langtag: 'entities.ormdepestman.zwbzmx_grid.columns.numcompilation',
            show: true,
            util: 'PX'
        },
        {
            name: 'createman',
            label: '建立人',
            langtag: 'entities.ormdepestman.zwbzmx_grid.columns.createman',
            show: true,
            util: 'PX'
        },
        {
            name: 'createdate',
            label: '建立时间',
            langtag: 'entities.ormdepestman.zwbzmx_grid.columns.createdate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        ...
        ormdutyid: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务ID 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务ID 值不能为空', trigger: 'blur' },
        ],
        ormzwbzmc: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务编制 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务编制 值不能为空', trigger: 'blur' },
        ],
        numcompilation: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '人数编制 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '人数编制 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '部门编制管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '部门编制管理标识 值不能为空', trigger: 'blur' },
        ],
        ormzwbzid: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务编制ID 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务编制ID 值不能为空', trigger: 'blur' },
        ],
        ormdutyname: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMDepEstManZWGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMDepEstManZWGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormdepestmanname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormdepestman: keys.join(';') }),Object.assign({ ormdepestman: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormdepestman,$event.ormdepestman);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormdepestman_zwbzmx_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormdepestman_zwbzmx_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMDepEstManZWGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMDepEstManZWGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormdepestman){
                    Object.assign(this.context,{ormdepestman:item.ormdepestman})
                }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMDepEstManZWGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormdepestman,args.row.ormdepestman);
        });
