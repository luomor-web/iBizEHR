...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormtitlename')">
                <el-table-column show-overflow-tooltip :prop="'ormtitlename'" :label="$t('entities.ormtitle.main_grid.columns.ormtitlename')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
                                <app-span name='ormtitlename' editorType="TEXTBOX" :value="row.ormtitlename"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('titletype')">
                <el-table-column show-overflow-tooltip :prop="'titletype'" :label="$t('entities.ormtitle.main_grid.columns.titletype')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.titletype" tag='ORMCL_POST_TYPE' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.ormtitle.main_grid.columns.createman')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.ormtitle.main_grid.columns.createdate')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormtitlename',
            label: '头衔',
            langtag: 'entities.ormtitle.main_grid.columns.ormtitlename',
            show: true,
            util: 'PX'
        },
        {
            name: 'titletype',
            label: '头衔类型',
            langtag: 'entities.ormtitle.main_grid.columns.titletype',
            show: true,
            util: 'PX'
        },
        {
            name: 'createman',
            label: '建立人',
            langtag: 'entities.ormtitle.main_grid.columns.createman',
            show: true,
            util: 'PX'
        },
        {
            name: 'createdate',
            label: '建立时间',
            langtag: 'entities.ormtitle.main_grid.columns.createdate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        titletype: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔类型 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔类型 值不能为空', trigger: 'blur' },
        ],
        ormtitlename: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '头衔标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMTITLEPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMTITLEPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormtitlename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormtitle: keys.join(';') }),Object.assign({ ormtitle: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormtitle,$event.ormtitle);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormtitle_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormtitle_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMTITLEPickupGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMTITLEPickupGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormtitle){
                    Object.assign(this.context,{ormtitle:item.ormtitle})
                }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormtitle,args.row.ormtitle);
        });
