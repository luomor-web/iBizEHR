...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('yyflag')">
                <el-table-column show-overflow-tooltip :prop="'yyflag'" :label="$t('entities.ormerporg.neworg_grid.columns.yyflag')" :width="120"  :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('ormerporgname')">
                <el-table-column show-overflow-tooltip :prop="'ormerporgname'" :label="$t('entities.ormerporg.neworg_grid.columns.ormerporgname')" :min-width="350"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormerporg.neworg_grid.columns.orgcode')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgtype')">
                <el-table-column show-overflow-tooltip :prop="'orgtype'" :label="$t('entities.ormerporg.neworg_grid.columns.orgtype')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgtypename')">
                <el-table-column show-overflow-tooltip :prop="'orgtypename'" :label="$t('entities.ormerporg.neworg_grid.columns.orgtypename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('belongou')">
                <el-table-column show-overflow-tooltip :prop="'belongou'" :label="$t('entities.ormerporg.neworg_grid.columns.belongou')" :width="340"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
import NewOrgService from './new-org-grid-service';
...
export default class NewOrgBase extends Vue implements ControlInterface {
     ...
     * @memberof NewOrg
     */
    public service: NewOrgService = new NewOrgService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'yyflag',
            label: '引用标志',
            langtag: 'entities.ormerporg.neworg_grid.columns.yyflag',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormerporgname',
            label: '组织名称',
            langtag: 'entities.ormerporg.neworg_grid.columns.ormerporgname',
            show: true,
            util: 'STAR'
        },
        {
            name: 'orgcode',
            label: '组织编码',
            langtag: 'entities.ormerporg.neworg_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtype',
            label: '组织类型',
            langtag: 'entities.ormerporg.neworg_grid.columns.orgtype',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtypename',
            label: '组织类型释义',
            langtag: 'entities.ormerporg.neworg_grid.columns.orgtypename',
            show: true,
            util: 'PX'
        },
        {
            name: 'belongou',
            label: '所属OU',
            langtag: 'entities.ormerporg.neworg_grid.columns.belongou',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGSubOrgsectorGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGSubOrgsectorGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormerporg_neworg_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormerporg_neworg_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMERPORGSubOrgsectorGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMERPORGSubOrgsectorGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormerporg){
