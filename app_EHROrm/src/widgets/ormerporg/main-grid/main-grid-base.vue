...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('yyflag')">
                <el-table-column show-overflow-tooltip :prop="'yyflag'" :label="$t('entities.ormerporg.main_grid.columns.yyflag')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.yyflag" tag='PIMCL_SFLJ' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormerporgname')">
                <el-table-column show-overflow-tooltip :prop="'ormerporgname'" :label="$t('entities.ormerporg.main_grid.columns.ormerporgname')" :min-width="350"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormerporg.main_grid.columns.orgcode')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgtype')">
                <el-table-column show-overflow-tooltip :prop="'orgtype'" :label="$t('entities.ormerporg.main_grid.columns.orgtype')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('orgtypename')">
                <el-table-column show-overflow-tooltip :prop="'orgtypename'" :label="$t('entities.ormerporg.main_grid.columns.orgtypename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.orgtypename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('belongou')">
                <el-table-column show-overflow-tooltip :prop="'belongou'" :label="$t('entities.ormerporg.main_grid.columns.belongou')" :width="280"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.belongou}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('numcomp')">
                <el-table-column show-overflow-tooltip :prop="'numcomp'" :label="$t('entities.ormerporg.main_grid.columns.numcomp')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='numcomp' editorType="TEXTBOX" :value="row.numcomp"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    public minorSortPSDEF: string = 'yyflag';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'yyflag',
            label: '引用标志',
            langtag: 'entities.ormerporg.main_grid.columns.yyflag',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormerporgname',
            label: '组织名称',
            langtag: 'entities.ormerporg.main_grid.columns.ormerporgname',
            show: true,
            util: 'STAR'
        },
        {
            name: 'orgcode',
            label: '组织编码',
            langtag: 'entities.ormerporg.main_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtype',
            label: '组织类型',
            langtag: 'entities.ormerporg.main_grid.columns.orgtype',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtypename',
            label: '组织类型释义',
            langtag: 'entities.ormerporg.main_grid.columns.orgtypename',
            show: true,
            util: 'PX'
        },
        {
            name: 'belongou',
            label: '所属OU',
            langtag: 'entities.ormerporg.main_grid.columns.belongou',
            show: true,
            util: 'PX'
        },
        {
            name: 'numcomp',
            label: '人数编制',
            langtag: 'entities.ormerporg.main_grid.columns.numcomp',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGOrgsectorGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGOrgsectorGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormerporg: keys.join(';') }),Object.assign({ ormerporg: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormerporg_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormerporg_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMERPORGOrgsectorGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMERPORGOrgsectorGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormerporg){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGOrgsectorGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
