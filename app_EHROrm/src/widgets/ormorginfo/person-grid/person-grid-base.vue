...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormorginfo.person_grid.columns.orgcode')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.orgcode}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormorginfoname')">
                <el-table-column show-overflow-tooltip :prop="'ormorginfoname'" :label="$t('entities.ormorginfo.person_grid.columns.ormorginfoname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.ormorginfoname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('belongregion')">
                <el-table-column show-overflow-tooltip :prop="'belongregion'" :label="$t('entities.ormorginfo.person_grid.columns.belongregion')" :min-width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('substandard')">
                <el-table-column show-overflow-tooltip :prop="'substandard'" :label="$t('entities.ormorginfo.person_grid.columns.substandard')" :width="140"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.substandard" tag='ORMCL_QY' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('postinfo')">
                <el-table-column show-overflow-tooltip :prop="'postinfo'" :label="$t('entities.ormorginfo.person_grid.columns.postinfo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.postinfo" tag='ORMCL_GW' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('shortname')">
                <el-table-column show-overflow-tooltip :prop="'shortname'" :label="$t('entities.ormorginfo.person_grid.columns.shortname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
import PersonService from './person-grid-service';
...
export default class PersonBase extends Vue implements ControlInterface {
     ...
     * @memberof Person
     */
    public service: PersonService = new PersonService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'orgcode',
            label: '员工编号',
            langtag: 'entities.ormorginfo.person_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorginfoname',
            label: '员工姓名',
            langtag: 'entities.ormorginfo.person_grid.columns.ormorginfoname',
            show: true,
            util: 'PX'
        },
        {
            name: 'belongregion',
            label: '证件号码',
            langtag: 'entities.ormorginfo.person_grid.columns.belongregion',
            show: true,
            util: 'STAR'
        },
        {
            name: 'substandard',
            label: '手机号码',
            langtag: 'entities.ormorginfo.person_grid.columns.substandard',
            show: true,
            util: 'PX'
        },
        {
            name: 'postinfo',
            label: '岗位',
            langtag: 'entities.ormorginfo.person_grid.columns.postinfo',
            show: true,
            util: 'PX'
        },
        {
            name: 'shortname',
            label: '员工状态',
            langtag: 'entities.ormorginfo.person_grid.columns.shortname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGInfoSubSubOrgGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGInfoSubSubOrgGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormorginfoname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormorginfo: keys.join(';') }),Object.assign({ ormorginfo: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormorginfo,$event.ormorginfo);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormorginfo_person_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormorginfo_person_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMORGInfoSubSubOrgGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMORGInfoSubSubOrgGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormorginfo){
                    Object.assign(this.context,{ormorginfo:item.ormorginfo})
                }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormorginfo,args.row.ormorginfo);
        });
