...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('range')">
                <el-table-column show-overflow-tooltip :prop="'range'" :label="$t('entities.ormbmkqdz.main_2_grid.columns.range')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='ATTCL_Range' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.range" tag='ATTCL_Range' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
