...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormpostlibname')">
                <el-table-column show-overflow-tooltip :prop="'ormpostlibname'" :label="$t('entities.ormpostdetails.main_grid.columns.ormpostlibname')" :min-width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='ormpostlib' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'ormpost-lib-redirect-view', height: 0,width: 0,title: $t('entities.ormpostlib.views.redirectview.title'),placement: '', isRedirectView: true,deResParameters: [
            ]
            ,parameters: [
            { pathName: 'ormpostlibs', parameterName: 'ormpostlib' },
            { pathName: 'redirectview', parameterName: 'redirectview' }
            ]}" valueitem="ormpostlibid">
                            <span>{{row.ormpostlibname}}</span>
                        </app-column-link >
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('gwtype')">
                <el-table-column show-overflow-tooltip :prop="'gwtype'" :label="$t('entities.ormpostdetails.main_grid.columns.gwtype')" :width="160"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('sugexcyear')">
                <el-table-column show-overflow-tooltip :prop="'sugexcyear'" :label="$t('entities.ormpostdetails.main_grid.columns.sugexcyear')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.sugexcyear}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('mustexcyear')">
                <el-table-column show-overflow-tooltip :prop="'mustexcyear'" :label="$t('entities.ormpostdetails.main_grid.columns.mustexcyear')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.mustexcyear}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('isconfidential')">
                <el-table-column show-overflow-tooltip :prop="'isconfidential'" :label="$t('entities.ormpostdetails.main_grid.columns.isconfidential')" :width="140"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.isconfidential" tag='PIMCL_SFLJ' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('postnature')">
                <el-table-column show-overflow-tooltip :prop="'postnature'" :label="$t('entities.ormpostdetails.main_grid.columns.postnature')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('gwflag')">
                <el-table-column show-overflow-tooltip :prop="'gwflag'" :label="$t('entities.ormpostdetails.main_grid.columns.gwflag')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.gwflag" tag='ORMCL_GWFL' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.ormpostdetails.main_grid.columns.createman')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.ormpostdetails.main_grid.columns.createdate')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormpostlibname',
            label: '岗位名称',
            langtag: 'entities.ormpostdetails.main_grid.columns.ormpostlibname',
            show: true,
            util: 'STAR'
        },
        {
            name: 'gwtype',
            label: '岗位类型',
            langtag: 'entities.ormpostdetails.main_grid.columns.gwtype',
            show: true,
            util: 'PX'
        },
        {
            name: 'sugexcyear',
            label: '建议交流年限',
            langtag: 'entities.ormpostdetails.main_grid.columns.sugexcyear',
            show: true,
            util: 'PX'
        },
        {
            name: 'mustexcyear',
            label: '必须交流年限',
            langtag: 'entities.ormpostdetails.main_grid.columns.mustexcyear',
            show: true,
            util: 'PX'
        },
        {
            name: 'isconfidential',
            label: '是否保密岗',
            langtag: 'entities.ormpostdetails.main_grid.columns.isconfidential',
            show: true,
            util: 'PX'
        },
        {
            name: 'postnature',
            label: '岗位性质',
            langtag: 'entities.ormpostdetails.main_grid.columns.postnature',
            show: true,
            util: 'PX'
        },
        {
            name: 'gwflag',
            label: '岗位标签',
            langtag: 'entities.ormpostdetails.main_grid.columns.gwflag',
            show: true,
            util: 'PX'
        },
        {
            name: 'createman',
            label: '建立人',
            langtag: 'entities.ormpostdetails.main_grid.columns.createman',
            show: true,
            util: 'PX'
        },
        {
            name: 'createdate',
            label: '建立时间',
            langtag: 'entities.ormpostdetails.main_grid.columns.createdate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPostDetailsGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPostDetailsGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormpostdetailsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormpostdetails: keys.join(';') }),Object.assign({ ormpostdetails: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormpostdetails,$event.ormpostdetails);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormpostdetails_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormpostdetails_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMPostDetailsGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMPostDetailsGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormpostdetails){
                    Object.assign(this.context,{ormpostdetails:item.ormpostdetails})
                }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormpostdetails,args.row.ormpostdetails);
        });
