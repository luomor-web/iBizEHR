...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('xh')">
                <el-table-column show-overflow-tooltip :prop="'xh'" :label="$t('entities.ormxmbmx.main_grid.columns.xh')" :width="100"  :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.ormxmbmx.main_grid.columns.pimpersonname')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('xmrylx')">
                <el-table-column show-overflow-tooltip :prop="'xmrylx'" :label="$t('entities.ormxmbmx.main_grid.columns.xmrylx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.xmrylx" tag='PIMCL_XMRYLX' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zsmc')">
                <el-table-column show-overflow-tooltip :prop="'zsmc'" :label="$t('entities.ormxmbmx.main_grid.columns.zsmc')" :width="170"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='zsmc' editorType="TEXTBOX" :value="row.zsmc"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zsbh')">
                <el-table-column show-overflow-tooltip :prop="'zsbh'" :label="$t('entities.ormxmbmx.main_grid.columns.zsbh')" :width="160"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='zsbh' editorType="TEXTBOX" :value="row.zsbh"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'xh',
            label: '排序号',
            langtag: 'entities.ormxmbmx.main_grid.columns.xh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.ormxmbmx.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'xmrylx',
            label: '项目人员类型',
            langtag: 'entities.ormxmbmx.main_grid.columns.xmrylx',
            show: true,
            util: 'PX'
        },
        {
            name: 'zsmc',
            label: '证书名称',
            langtag: 'entities.ormxmbmx.main_grid.columns.zsmc',
            show: true,
            util: 'PX'
        },
        {
            name: 'zsbh',
            label: '证书编号',
            langtag: 'entities.ormxmbmx.main_grid.columns.zsbh',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        ...
        zsmc: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书名称 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书名称 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '项目部明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '项目部明细标识 值不能为空', trigger: 'blur' },
        ],
        pimpersonname: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMXMBMXGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMXMBMXGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormxmbmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormxmbmx: keys.join(';') }),Object.assign({ ormxmbmx: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormxmbmx,$event.ormxmbmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormxmbmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormxmbmx_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMXMBMXGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMXMBMXGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormxmbmx){
                    Object.assign(this.context,{ormxmbmx:item.ormxmbmx})
                }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMXMBMXGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormxmbmx,args.row.ormxmbmx);
        });
