    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.ormpost.gwchoice_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTGWBZXZPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTGWBZXZPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormpost: keys.join(';') }),Object.assign({ ormpost: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMPOSTGWBZXZPickupGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ORMPOSTGWBZXZPickupGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ormpost){
