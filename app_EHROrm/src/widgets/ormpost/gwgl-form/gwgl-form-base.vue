<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='ormpost_gwgl' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.ormpost.gwgl_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.xh.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='xh' :itemRules="this.rules.xh" class='' :caption="$t('entities.ormpost.gwgl_form.details.xh')" uiStyle="DEFAULT" :labelWidth="160" :isShowCaption="true" :error="detailsModel.xh.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.gwtype.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    ...
    <app-checkbox-list v-model="data.gwtype" :disabled="detailsModel.gwtype.disabled"  tag='ORMCL_GWTYPE' codelistType='DYNAMIC'   mode="" name="gwtype" style=""></app-checkbox-list>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.lgnx.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='lgnx' :itemRules="this.rules.lgnx" class='' :caption="$t('entities.ormpost.gwgl_form.details.lgnx')" uiStyle="DEFAULT" :labelWidth="160" :isShowCaption="true" :error="detailsModel.lgnx.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.lgnx"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.lgnx.disabled" type='number'  style=""></input-box>
</app-form-item>
...
export default class GWGLBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        lgnx: [
            { type: 'number', message: '轮岗年限 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '轮岗年限 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '轮岗年限 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '轮岗年限 值不能为空', trigger: 'blur' },
        ],
        isconfidential: [
        ...
        qzlgnx: [
            { type: 'number', message: '强制轮岗年限 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '强制轮岗年限 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '强制轮岗年限 值不能为空', trigger: 'change' },
...
, 
        lgnx: new FormItemModel({ caption: '轮岗年限', detailType: 'FORMITEM', name: 'lgnx', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    onLgnxChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'lgnx', newVal: newVal, oldVal: oldVal });
    }
    ...
    onQzlgnxChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'qzlgnx', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'ormpost_gwgl',popTitle:'标准岗位'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ORMPOSTEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
