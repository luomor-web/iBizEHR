<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='trmtrainactment_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.trmtrainactment.main_form.details.group1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        pxjg: [
            ...
            { type: 'string', message: '评估结果 值必须为字符串类型', trigger: 'blur' },
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '培训活动评估信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'entities.trmtrainactment.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
    ...
    /**
     * 监控表单属性 trmtrainactapplyid 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof Main
     */
    @Watch('data.trmtrainactapplyid')
    onTrmtrainactapplyidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'trmtrainactapplyid', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.dcwjtm')
    onDcwjtmChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'dcwjtm', newVal: newVal, oldVal: oldVal });
    }
    ...
    /**
     * 监控表单属性 trmtrainactmentid 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof Main
     */
    @Watch('data.trmtrainactmentid')
    onTrmtrainactmentidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'trmtrainactmentid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.trmtrainactment){
            Object.assign(this.context,{trmtrainactment:data.trmtrainactment})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.trmtrainactment = copyData.srfkey;
        copyData.trmtrainactmentid = copyData.srfkey;
        Object.assign(this.context,{trmtrainactment:copyData.trmtrainactment})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'trmtrainactment_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainActmentEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainActmentEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'TrmTrainActmentEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'TrmTrainActmentEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.trmtrainactment || Object.is(arg.trmtrainactment, '')) {
            return;
        }
