...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ehr-app-ehrtrm\/dynacharts\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ehr-app-ehrtrm\/dynacharts\/process-definitions-nodes$/)).reply((config: any) => {
...
// Select
mock.onGet(new RegExp(/^\/dynacharts\/([a-zA-Z0-9\-\;]{1,35})\/select$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynachart 方法: Select");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const matchArray:any = new RegExp(/^\/dynacharts\/([a-zA-Z0-9\-\;]{1,35})\/select$/).exec(config.url);
    let tempValue: any = {};
...
// Update
mock.onPut(new RegExp(/^\/dynacharts\/?([a-zA-Z0-9\-\;]{0,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynachart 方法: Update");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// Save
mock.onPost(new RegExp(/^\/dynacharts\/?([a-zA-Z0-9\-\;]{0,35})\/save$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynachart 方法: Save");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['dynachartid'];
    const matchArray:any = new RegExp(/^\/dynacharts\/([a-zA-Z0-9\-\;]{1,35})\/save$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FetchDefault
mock.onGet(new RegExp(/^\/dynacharts\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynachart 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDefault
mock.onGet(new RegExp(/^\/dynacharts\/fetchdefault(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynachart 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
