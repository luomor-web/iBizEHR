<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='salstdgl_main' style="">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfkey: [
            ...
            { required: false, type: 'string', message: '工龄工资标准标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '工龄工资标准标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '工龄工资标准名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '工龄工资标准名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '工龄工资标准名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '工龄工资标准名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        qsgl: [
            ...
            { required: true, type: 'number', message: '起始年功工龄 值不能为空', trigger: 'change' },
            { required: true, type: 'number', message: '起始年功工龄 值不能为空', trigger: 'blur' },
        ],
        jsgl: [
            { type: 'number', message: '结束年功工龄 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '结束年功工龄 值必须为数值类型', trigger: 'blur' },
            { required: true, type: 'number', message: '结束年功工龄 值不能为空', trigger: 'change' },
            { required: true, type: 'number', message: '结束年功工龄 值不能为空', trigger: 'blur' },
        ],
        bz: [
...
, 
        srfkey: new FormItemModel({ caption: '工龄工资标准标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '工龄工资标准名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.qsgl')
    onQsglChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'qsgl', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.jsgl')
    onJsglChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'jsgl', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof MainBase
     */
    @Watch('data.salstdglid')
    onSalstdglidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'salstdglid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.salstdgl){
            Object.assign(this.context,{salstdgl:data.salstdgl})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.salstdgl = copyData.srfkey;
        copyData.salstdglid = copyData.srfkey;
        Object.assign(this.context,{salstdgl:copyData.salstdgl})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'salstdgl_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdglEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdglEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'SalStdglEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'SalStdglEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.salstdgl || Object.is(arg.salstdgl, '')) {
            return;
        }
