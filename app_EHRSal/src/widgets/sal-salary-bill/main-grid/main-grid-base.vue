...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salsalarybillname')">
                <el-table-column show-overflow-tooltip :prop="'salsalarybillname'" :label="$t('entities.salsalarybill.main_grid.columns.salsalarybillname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.salsalarybillname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salsalarybillname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salsalarybill.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.salsalarybill.main_grid.columns.ormorgsectorname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ssou')">
                <el-table-column show-overflow-tooltip :prop="'ssou'" :label="$t('entities.salsalarybill.main_grid.columns.ssou')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.ssou')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ssou}}</span>
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.salsalarybill.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.salsalarybill.main_grid.columns.updatedate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalarybill.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    public appEntityService: SalSalaryBillService = new SalSalaryBillService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '工资表名称',
            langtag: 'entities.salsalarybill.main_grid.columns.salsalarybillname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salsalarybill.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '部门',
            langtag: 'entities.salsalarybill.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'px'
        },
        {
            name: 'ssou',
            label: '所属OU',
            langtag: 'entities.salsalarybill.main_grid.columns.ssou',
            show: true,
            util: 'px'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.salsalarybill.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.salsalarybill.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资表标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资表标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryBillPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryBillPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salsalarybillname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salsalarybill: _keys }),Object.assign({ salsalarybill: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salsalarybill,$event.salsalarybill);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salsalarybill_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salsalarybill_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSalaryBillPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSalaryBillPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salsalarybill){
                            Object.assign(this.context,{salsalarybill:item.salsalarybill});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salsalarybill,args.row.salsalarybill);
        });
