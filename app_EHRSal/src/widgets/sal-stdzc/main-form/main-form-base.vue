<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='salstdzc_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.salstdzc.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.ormorgname.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='ormorgname' :itemRules="this.rules.ormorgname" class='' :caption="$t('entities.salstdzc.main_form.details.ormorgname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.ormorgname.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.zcdj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='zcdj' :itemRules="this.rules.zcdj" class='' :caption="$t('entities.salstdzc.main_form.details.zcdj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.zcdj.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.bz.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='bz' :itemRules="this.rules.bz" class='' :caption="$t('entities.salstdzc.main_form.details.bz')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.bz.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfkey: [
            { type: 'string', message: '技术津贴标准标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '技术津贴标准标识 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '技术津贴标准标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '技术津贴标准标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '技术津贴标准名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '技术津贴标准名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '技术津贴标准名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '技术津贴标准名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        zcdj: [
            ...
            { required: true, type: 'string', message: '职称等级 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '职称等级 值不能为空', trigger: 'blur' },
        ],
        bz: [
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '技术津贴标准信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.salstdzc.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfkey: new FormItemModel({ caption: '技术津贴标准标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '技术津贴标准名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        salstdzcid: new FormItemModel({ caption: '技术津贴标准标识', detailType: 'FORMITEM', name: 'salstdzcid', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof MainBase
     */
    @Watch('data.salstdzcid')
    onSalstdzcidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'salstdzcid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.salstdzc){
            Object.assign(this.context,{salstdzc:data.salstdzc})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.salstdzc = copyData.srfkey;
        copyData.salstdzcid = copyData.srfkey;
        Object.assign(this.context,{salstdzc:copyData.salstdzc})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'salstdzc_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdzcEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdzcEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'SalStdzcEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'SalStdzcEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.salstdzc || Object.is(arg.salstdzc, '')) {
            return;
        }
