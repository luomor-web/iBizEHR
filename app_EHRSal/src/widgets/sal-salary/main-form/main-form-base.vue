<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='salsalary_main' style="">
...
<i-col v-show="detailsModel.grouppanel1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.grouppanel1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.salsalary.main_form.details.grouppanel1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
    ...
    paramItem='salsalary' 
    :parentdata='{"srfparentdefname":"SALSALARYID","srfparentdename":"SALSALARY","SRFPARENTTYPE":"DER1N","srfparentmode":"DER1N_SALSALARYDETAIL_SALSALARY_SALSALARYID","SRFDER1NID":"DER1N_SALSALARYDETAIL_SALSALARY_SALSALARYID"}'
    :parameters="[
    ]"
    :context="context"
    :viewparams="viewparams"
    :localContext ='{}'
    :localParam ='{}'
    parameterName='salsalary'
    parentName="SalSalary"  
    refviewtype='DEGRIDVIEW' 
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='sal-salary-detail-grid-view' 
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfmajortext: [
            { type: 'string', message: '员工薪酬名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '员工薪酬名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '员工薪酬名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '员工薪酬名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        xc: [
            ...
            { required: false, type: 'number', message: '实发薪酬 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '实发薪酬 值不能为空', trigger: 'blur' },
        ],
        state: [
...
, 
        grouppanel1: new FormGroupPanelModel({ caption: '薪酬明细', detailType: 'GROUPPANEL', name: 'grouppanel1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'entities.salsalary.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfkey: new FormItemModel({ caption: '员工薪酬标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '员工薪酬名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        ormdutyname: new FormItemModel({ caption: '职务', detailType: 'FORMITEM', name: 'ormdutyname', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
...
, 
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.salsalary){
            Object.assign(this.context,{salsalary:data.salsalary})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.salsalary = copyData.srfkey;
        copyData.salsalaryid = copyData.srfkey;
        Object.assign(this.context,{salsalary:copyData.salsalary})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'salsalary_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'SalSalaryEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'SalSalaryEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.salsalary || Object.is(arg.salsalary, '')) {
            return;
        }
