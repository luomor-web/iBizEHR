...
import cn.ibizlab.ehr.core.pcm.service.logic.IPcmProfileGroupConfirmLogic;
import cn.ibizlab.ehr.core.pcm.domain.PcmProfile;

/**
 * 关系型数据实体[GroupConfirm] 对象
 */
@Slf4j
@Service
public class PcmProfileGroupConfirmLogicImpl implements IPcmProfileGroupConfirmLogic{
    ...
    public void execute(PcmProfile et){
        ...
        try{
           ...
           kieSession.setGlobal("pcmprofilegroupconfirmdefault",et);
           ...
           kieSession.startProcess("cn.ibizlab.ehr.core.pcm.service.logic.pcmprofilegroupconfirm");

        }catch(Exception e){
            throw new RuntimeException("执行[总部审批-同意]处理逻辑发生异常"+e);
        }finally {
            if(kieSession!=null)
            kieSession.destroy();
        }
