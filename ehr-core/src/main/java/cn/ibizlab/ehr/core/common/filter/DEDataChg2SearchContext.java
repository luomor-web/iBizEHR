...
/**
 * 关系型数据实体[DEDataChg2] 查询条件对象
 */
@Slf4j
@Data
public class DEDataChg2SearchContext extends QueryWrapperContext<DEDataChg2> {

	private String n_dedatachg2name_like;//[实体数据变更2名称]
	public void setN_dedatachg2name_like(String n_dedatachg2name_like) {
        this.n_dedatachg2name_like = n_dedatachg2name_like;
        if(!ObjectUtils.isEmpty(this.n_dedatachg2name_like)){
            this.getSelectCond().like("dedatachg2name", n_dedatachg2name_like);
        }
    }
	...
	public void setQuery(String query)
	{
		 ...
		 if(!StringUtils.isEmpty(query)){
			this.getSelectCond().or().like("dedatachg2name",query);
		 }
	}
}
