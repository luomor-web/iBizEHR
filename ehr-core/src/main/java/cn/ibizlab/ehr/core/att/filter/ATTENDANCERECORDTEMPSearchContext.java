...
public class ATTENDANCERECORDTEMPSearchContext extends QueryWrapperContext<ATTENDANCERECORDTEMP> {
	...
	private String n_attendancerecordtempname_like;//[考勤记录(中间表)名称]
	public void setN_attendancerecordtempname_like(String n_attendancerecordtempname_like) {
        this.n_attendancerecordtempname_like = n_attendancerecordtempname_like;
        if(!ObjectUtils.isEmpty(this.n_attendancerecordtempname_like)){
            this.getSelectCond().like("attendancerecordtempname", n_attendancerecordtempname_like);
        }
    }
	private String n_isweekend_eq;//[是否周末]
	public void setN_isweekend_eq(String n_isweekend_eq) {
        ...
        if(!ObjectUtils.isEmpty(this.n_isweekend_eq)){
            this.getSelectCond().eq("isweekend", n_isweekend_eq);
        }
    }
	private String n_rshd_eq;//[人事核对]
	public void setN_rshd_eq(String n_rshd_eq) {
        this.n_rshd_eq = n_rshd_eq;
        if(!ObjectUtils.isEmpty(this.n_rshd_eq)){
            this.getSelectCond().eq("rshd", n_rshd_eq);
        }
    }
