...
public class UserDict extends EntityMP implements Serializable {
    ...
    @TableId(value= "userdictid",type=IdType.UUID)
    ...
    public String getUserdictid(){
        if(ObjectUtils.isEmpty(userdictid)){
            userdictid=(String)getDefaultKey(true);
        }
        return userdictid;
    }
    ...
    public Serializable getDefaultKey(boolean gen) {
        if((!ObjectUtils.isEmpty(this.getOwnertype()))&&(!ObjectUtils.isEmpty(this.getOwnerid())))
            return DigestUtils.md5DigestAsHex(String.format("%s||%s" ,this.getOwnertype(),this.getOwnerid()).getBytes());
        return null;
    }
}
