...
import cn.ibizlab.ehr.core.pcm.service.logic.IPcmProfileCompanyConfirmLogic;
import cn.ibizlab.ehr.core.pcm.domain.PcmProfile;

/**
 * 关系型数据实体[CompanyConfirm] 对象
 */
@Slf4j
@Service
public class PcmProfileCompanyConfirmLogicImpl implements IPcmProfileCompanyConfirmLogic{
    ...
    public void execute(PcmProfile et){
        ...
        try{
           ...
           kieSession.setGlobal("pcmprofilecompanyconfirmdefault",et);
           ...
           kieSession.startProcess("cn.ibizlab.ehr.core.pcm.service.logic.pcmprofilecompanyconfirm");

        }catch(Exception e){
            throw new RuntimeException("执行[公司审批-同意]处理逻辑发生异常"+e);
        }finally {
            if(kieSession!=null)
            kieSession.destroy();
        }
