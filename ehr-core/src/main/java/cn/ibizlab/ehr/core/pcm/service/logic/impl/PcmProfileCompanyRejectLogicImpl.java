...
import cn.ibizlab.ehr.core.pcm.service.logic.IPcmProfileCompanyRejectLogic;
import cn.ibizlab.ehr.core.pcm.domain.PcmProfile;

/**
 * 关系型数据实体[CompanyReject] 对象
 */
@Slf4j
@Service
public class PcmProfileCompanyRejectLogicImpl implements IPcmProfileCompanyRejectLogic{
    ...
    public void execute(PcmProfile et){
        ...
        try{
           ...
           kieSession.setGlobal("pcmprofilecompanyrejectdefault",et);
           ...
           kieSession.startProcess("cn.ibizlab.ehr.core.pcm.service.logic.pcmprofilecompanyreject");

        }catch(Exception e){
            throw new RuntimeException("执行[公司审批-拒绝]处理逻辑发生异常"+e);
        }finally {
            if(kieSession!=null)
            kieSession.destroy();
        }
