...
/**
 * 关系型数据实体[DataSyncOut] 查询条件对象
 */
@Slf4j
@Data
public class DataSyncOutSearchContext extends QueryWrapperContext<DataSyncOut> {
	...
	private String n_datasyncoutname_like;//[数据同步发送队列名称]
	public void setN_datasyncoutname_like(String n_datasyncoutname_like) {
        this.n_datasyncoutname_like = n_datasyncoutname_like;
        if(!ObjectUtils.isEmpty(this.n_datasyncoutname_like)){
            this.getSelectCond().like("datasyncoutname", n_datasyncoutname_like);
        }
    }
	...
	public void setQuery(String query)
	{
		 ...
		 if(!StringUtils.isEmpty(query)){
			this.getSelectCond().or().like("datasyncoutname",query);
		 }
	}
}
