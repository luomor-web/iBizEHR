package cn.ibizlab.ehr.core.common.domain;

import java.sql.Timestamp;
import java.util.ArrayList;
import java.util.List;
import java.util.Map;
import java.math.BigInteger;
import java.util.HashMap;
import java.math.BigDecimal;
import com.alibaba.fastjson.annotation.JSONField;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonFormat;
import org.springframework.util.ObjectUtils;
import org.springframework.util.DigestUtils;
import cn.ibizlab.ehr.util.domain.EntityBase;
import cn.ibizlab.ehr.util.annotation.DEField;
import cn.ibizlab.ehr.util.enums.DEPredefinedFieldType;
import cn.ibizlab.ehr.util.enums.DEFieldDefaultValueType;
import java.io.Serializable;
import lombok.Data;
import org.springframework.data.annotation.Transient;


import com.baomidou.mybatisplus.annotation.*;
import cn.ibizlab.ehr.util.domain.EntityMP;


/**
 * 实体[数据对象能力明细]
 */
@Data
@TableName(value = "T_SRFUSERROLEDATADETAIL",resultMap = "UserRoleDataDetailResultMap")
public class UserRoleDataDetail extends EntityMP implements Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 数据对象能力明细标识
     */
    @DEField(isKeyField=true)
    @TableId(value= "userroledatadetailid",type=IdType.UUID)
    @JSONField(name = "userroledatadetailid")
    @JsonProperty("userroledatadetailid")
    private String userroledatadetailid;
    /**
     * 更新时间
     */
    @DEField(preType = DEPredefinedFieldType.UPDATEDATE)
    @TableField(value = "updatedate")
    @JsonFormat(pattern="yyyy-MM-dd HH:mm:ss", locale = "zh" , timezone="GMT+8")
    @JSONField(name = "updatedate" , format="yyyy-MM-dd HH:mm:ss")
    @JsonProperty("updatedate")
    private Timestamp updatedate;
    /**
     * 保留字段
     */
    @TableField(value = "reserver")
    @JSONField(name = "reserver")
    @JsonProperty("reserver")
    private String reserver;
    /**
     * 建立时间
     */
    @DEField(preType = DEPredefinedFieldType.CREATEDATE)
    @TableField(value = "createdate" , fill = FieldFill.INSERT)
    @JsonFormat(pattern="yyyy-MM-dd HH:mm:ss", locale = "zh" , timezone="GMT+8")
    @JSONField(name = "createdate" , format="yyyy-MM-dd HH:mm:ss")
    @JsonProperty("createdate")
    private Timestamp createdate;
    /**
     * 保留字段3
     */
    @TableField(value = "reserver3")
    @JSONField(name = "reserver3")
    @JsonProperty("reserver3")
    private String reserver3;
    /**
     * 建立人
     */
    @DEField(preType = DEPredefinedFieldType.CREATEMAN)
    @TableField(value = "createman" , fill = FieldFill.INSERT)
    @JSONField(name = "createman")
    @JsonProperty("createman")
    private String createman;
    /**
     * 保留字段4
     */
    @TableField(value = "reserver4")
    @JSONField(name = "reserver4")
    @JsonProperty("reserver4")
    private String reserver4;
    /**
     * 更新人
     */
    @DEField(preType = DEPredefinedFieldType.UPDATEMAN)
    @TableField(value = "updateman")
    @JSONField(name = "updateman")
    @JsonProperty("updateman")
    private String updateman;
    /**
     * 数据对象能力明细名称
     */
    @DEField(defaultValue = "能力明细")
    @TableField(value = "userroledatadetailname")
    @JSONField(name = "userroledatadetailname")
    @JsonProperty("userroledatadetailname")
    private String userroledatadetailname;
    /**
     * 备注
     */
    @TableField(value = "memo")
    @JSONField(name = "memo")
    @JsonProperty("memo")
    private String memo;
    /**
     * 排除数据范围
     */
    @DEField(defaultValue = "0")
    @TableField(value = "isexclude")
    @JSONField(name = "isexclude")
    @JsonProperty("isexclude")
    private Integer isexclude;
    /**
     * 保留字段2
     */
    @TableField(value = "reserver2")
    @JSONField(name = "reserver2")
    @JsonProperty("reserver2")
    private String reserver2;
    /**
     * 数据对象能力
     */
    @TableField(exist = false)
    @JSONField(name = "userroledataname")
    @JsonProperty("userroledataname")
    private String userroledataname;
    /**
     * 查询模型
     */
    @TableField(exist = false)
    @JSONField(name = "querymodelname")
    @JsonProperty("querymodelname")
    private String querymodelname;
    /**
     * 数据对象能力
     */
    @TableField(value = "userroledataid")
    @JSONField(name = "userroledataid")
    @JsonProperty("userroledataid")
    private String userroledataid;
    /**
     * 查询模型
     */
    @TableField(value = "querymodelid")
    @JSONField(name = "querymodelid")
    @JsonProperty("querymodelid")
    private String querymodelid;

    /**
     * 
     */
    @JsonIgnore
    @JSONField(serialize = false)
    @TableField(exist = false)
    private cn.ibizlab.ehr.core.demodel.domain.QueryModel querymodel;

    /**
     * 
     */
    @JsonIgnore
    @JSONField(serialize = false)
    @TableField(exist = false)
    private cn.ibizlab.ehr.core.common.domain.UserRoleData userroledata;



    /**
     * 设置 [保留字段]
     */
    public void setReserver(String reserver){
        this.reserver = reserver ;
        this.modify("reserver",reserver);
    }
    /**
     * 设置 [保留字段3]
     */
    public void setReserver3(String reserver3){
        this.reserver3 = reserver3 ;
        this.modify("reserver3",reserver3);
    }
    /**
     * 设置 [保留字段4]
     */
    public void setReserver4(String reserver4){
        this.reserver4 = reserver4 ;
        this.modify("reserver4",reserver4);
    }
    /**
     * 设置 [数据对象能力明细名称]
     */
    public void setUserroledatadetailname(String userroledatadetailname){
        this.userroledatadetailname = userroledatadetailname ;
        this.modify("userroledatadetailname",userroledatadetailname);
    }
    /**
     * 设置 [备注]
     */
    public void setMemo(String memo){
        this.memo = memo ;
        this.modify("memo",memo);
    }
    /**
     * 设置 [排除数据范围]
     */
    public void setIsexclude(Integer isexclude){
        this.isexclude = isexclude ;
        this.modify("isexclude",isexclude);
    }
    /**
     * 设置 [保留字段2]
     */
    public void setReserver2(String reserver2){
        this.reserver2 = reserver2 ;
        this.modify("reserver2",reserver2);
    }
    /**
     * 设置 [数据对象能力]
     */
    public void setUserroledataid(String userroledataid){
        this.userroledataid = userroledataid ;
        this.modify("userroledataid",userroledataid);
    }
    /**
     * 设置 [查询模型]
     */
    public void setQuerymodelid(String querymodelid){
        this.querymodelid = querymodelid ;
        this.modify("querymodelid",querymodelid);
    }

    /**
     * 获取 [数据对象能力明细标识]
     */
    public String getUserroledatadetailid(){
        if(ObjectUtils.isEmpty(userroledatadetailid)){
            userroledatadetailid=(String)getDefaultKey(true);
        }
        return userroledatadetailid;
    }

    @Override
    public Serializable getDefaultKey(boolean gen) {
        if((!ObjectUtils.isEmpty(this.getUserroledataid()))&&(!ObjectUtils.isEmpty(this.getQuerymodelid())))
            return DigestUtils.md5DigestAsHex(String.format("%s||%s" ,this.getUserroledataid(),this.getQuerymodelid()).getBytes());
        return null;
    }
}


