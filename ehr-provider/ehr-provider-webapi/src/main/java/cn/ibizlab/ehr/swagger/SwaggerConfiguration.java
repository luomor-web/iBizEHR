...
public class SwaggerConfiguration {
	...
	@Bean
	public Docket saasadminDocket() {
		return new Docket(DocumentationType.SWAGGER_2)
				.groupName("SaaS应用管理接口")
				.pathMapping("/")
				.apiInfo(
						new ApiInfoBuilder()
						.title("SaaS应用管理接口")
				...
				.select()
				.apis(RequestHandlerSelectors.basePackage("cn.ibizlab.ehr.saasadmin"))
				//.paths(or(regex("/rest/saasadmin/.*")))
				.paths(PathSelectors.any())
				.build()
				;
	}
	@Bean
	public Docket parttimepersonDocket() {
		return new Docket(DocumentationType.SWAGGER_2)
				.groupName("兼职人员接口")
				.pathMapping("/")
				.apiInfo(
				...
				.select()
				.apis(RequestHandlerSelectors.basePackage("cn.ibizlab.ehr.parttimeperson"))
				//.paths(or(regex("/rest/parttimeperson/.*")))
				.paths(PathSelectors.any())
				.build()
				;
	}
	@Bean
	public Docket wfcallbackDocket() {
		return new Docket(DocumentationType.SWAGGER_2)
				.groupName("工作流引擎回调接口")
				.pathMapping("/")
				.apiInfo(
				...
				.select()
				.apis(RequestHandlerSelectors.basePackage("cn.ibizlab.ehr.wfcallback"))
				//.paths(or(regex("/rest/wfcallback/.*")))
				.paths(PathSelectors.any())
				.build()
				;
	}
	@Bean
	public Docket specialroleDocket() {
		return new Docket(DocumentationType.SWAGGER_2)
				.groupName("特殊角色接口")
				.pathMapping("/")
				.apiInfo(
				...
				.select()
				.apis(RequestHandlerSelectors.basePackage("cn.ibizlab.ehr.specialrole"))
				//.paths(or(regex("/rest/specialrole/.*")))
				.paths(PathSelectors.any())
				.build()
				;
	}
