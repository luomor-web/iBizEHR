...
export default class MainService extends ControlService {
     ...
     */
    public appEntityService: VacLeaveTipsService = new VacLeaveTipsService({ $store: this.getStore() });
        ...
        return new Promise((resolve: any, reject: any) => {
            ...
            result.then((response) => {
                ...
                if(response.data){
                    Object.assign(response.data,{srfuf:'0'});
                    //仿真主键数据
                    response.data.vacleavetipsid = Util.createUUID();
                }
    ...
    public mergeDefaults(response:any = {}){ 
        if(response.data){                    
            Object.assign(response.data,{'vacholidayrulesid':'srfparentkey'});
        }
    }
