<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pcmwzd0001_ygszkq' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pcmwzd0001.ygszkq_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.profilelist.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='profilelist' :itemRules="this.rules.profilelist" class='' :caption="$t('entities.pcmwzd0001.ygszkq_form.details.profilelist')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.profilelist.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.peoplenum.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='peoplenum' :itemRules="this.rules.peoplenum" class='' :caption="$t('entities.pcmwzd0001.ygszkq_form.details.peoplenum')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.peoplenum.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.attendencesetup.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='attendencesetup' :itemRules="this.rules.attendencesetup" class='' :caption="$t('entities.pcmwzd0001.ygszkq_form.details.attendencesetup')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.attendencesetup.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-mpicker
    :activeData="data"
    :disabled="detailsModel.attendencesetup.disabled"
    :curvalue="data.attendencesetup"
...
import YGSZKQService from './ygszkq-form-service';
...
export default class YGSZKQBase extends Vue implements ControlInterface {
     ...
     * @memberof YGSZKQ
     */
    public service: YGSZKQService = new YGSZKQService({ $store: this.$store });
     ...
     * @memberof YGSZKQ
     */
    public appEntityService: PcmWzd0001Service = new PcmWzd0001Service({ $store: this.$store });
    ...
    public rules: any = {
        ...
        attendencesetup: [
            { type: 'string', message: '考勤组 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考勤组 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '考勤组 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '考勤组 值不能为空', trigger: 'blur' },
        ],
        wzd0001id: [
            { type: 'string', message: '向导实体1标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '向导实体1标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        group1: new FormGroupPanelModel({ caption: '选择人员及时间', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.pcmwzd0001.ygszkq_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        attendencesetup: new FormItemModel({ caption: '考勤组', detailType: 'FORMITEM', name: 'attendencesetup', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 attendencesetup 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof YGSZKQ
     */
    @Watch('data.attendencesetup')
    onAttendencesetupChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'attendencesetup', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.pcmwzd0001){
            Object.assign(this.context,{pcmwzd0001:data.pcmwzd0001})
        }
     ...
     *
     * @param {*} [arg={}]
     * @memberof @memberof YGSZKQ
     */
    public copy(srfkey: string): void {
        let copyData = this.$store.getters.getCopyData(srfkey);
        copyData.srfkey = Util.createUUID();
        copyData.pcmwzd0001 = copyData.srfkey;
        copyData.wzd0001id = copyData.srfkey;
        Object.assign(this.context,{pcmwzd0001:copyData.pcmwzd0001})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pcmwzd0001_ygszkq',popTitle:'员工设置考勤'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmWzd0001YGSZKQOptionView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmWzd0001YGSZKQOptionView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PcmWzd0001YGSZKQOptionView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PcmWzd0001YGSZKQOptionView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.pcmwzd0001 || Object.is(arg.pcmwzd0001, '')) {
            return;
        }
