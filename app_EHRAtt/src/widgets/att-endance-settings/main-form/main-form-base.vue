<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='attendancesettings_main' style="">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfkey: [
            ...
            { required: false, type: 'string', message: '考勤人员标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考勤人员标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '考勤人员名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考勤人员名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考勤人员名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考勤人员名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
...
, 
        srfkey: new FormItemModel({ caption: '考勤人员标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '考勤人员名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     * @memberof MainBase
     */
    @Watch('data.attendancesettingsid')
    onAttendancesettingsidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'attendancesettingsid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        if(data.attendancesettings){
            Object.assign(this.context,{attendancesettings:data.attendancesettings})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.attendancesettings = copyData.srfkey;
        copyData.attendancesettingsid = copyData.srfkey;
        Object.assign(this.context,{attendancesettings:copyData.attendancesettings})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'attendancesettings_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.attendancesettings || Object.is(arg.attendancesettings, '')) {
            return;
        }
