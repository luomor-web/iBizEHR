<template>
    ...
    <el-steps class="wizard-steps"  :active="wizardForms.indexOf(activeForm)" finish-status="success">
      ...
      <el-step title="考勤部门"></el-step>
      <el-step title="考勤人员"></el-step>
    </el-steps>
...
import KQSZXDService from './kqszxd-wizardpanel-service';
...
export default class KQSZXDBase extends Vue implements ControlInterface {
     ...
     * @memberof KQSZXD
     */
    public service: KQSZXDService = new KQSZXDService({ $store: this.$store });
    ...
    public doInit(opt: any = {}) {
        ...
        post.then((response: any) => {
            if (response && response.status === 200) {
                ...
                if(response.data.attendencesetup){
                    Object.assign(this.context,{attendencesetup:response.data.attendencesetup})
                }
                this.formLoad();
            }
        }).catch((response: any) => {
...
<style lang='less'>
@import './kqszxd-wizardpanel.less';
</style>