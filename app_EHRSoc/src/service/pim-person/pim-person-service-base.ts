...
// 实体接口
    ...
    public async Update(context: any = {},data: any = {}, isloading?: boolean): Promise<any> {
        let masterData:any = {};
        let socarchivesData:any = [];
        if(!Object.is(this.tempStorage.getItem(context.srfsessionkey+'_socarchives'),'undefined')){
            socarchivesData = JSON.parse(this.tempStorage.getItem(context.srfsessionkey+'_socarchives') as any);
            if(socarchivesData && socarchivesData.length && socarchivesData.length > 0){
                socarchivesData.forEach((item:any) => {
                    if(item.srffrontuf){
                        if(Object.is(item.srffrontuf,"0")){
                            item.socarchivesid = null;
                        }
        ...
        masterData.socarchives = socarchivesData;
        Object.assign(data,masterData);
            let res:any = await  Http.getInstance().put(`/pimpeople/${context.pimperson}`,data,isloading);
            this.tempStorage.setItem(context.srfsessionkey+'_socarchives',JSON.stringify(res.data.socarchives));
            return res;
    }
    ...
    public async Create(context: any = {},data: any = {}, isloading?: boolean): Promise<any> {
        ...
        this.tempStorage.setItem(tempContext.srfsessionkey+'_socarchives',JSON.stringify(res.data.socarchives));
        return res;
    }
