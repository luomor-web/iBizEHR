<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='socaccount_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.socaccount.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
    ...
    <app-form-item name='socaccountname' :itemRules="this.rules.socaccountname" class='' :caption="$t('entities.socaccount.main_form.details.socaccountname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.socaccountname.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.socaccountname"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.socaccountname.disabled" type='text'  style=""></input-box>
</app-form-item>
...
<i-col v-show="detailsModel.ormorgname.visible" :style="{}"  :sm="{ span: 24, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 8, offset: 0 }" :xl="{ span: 8, offset: 0 }">
    <app-form-item name='ormorgname' :itemRules="this.rules.ormorgname" class='' :caption="$t('entities.socaccount.main_form.details.ormorgname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.ormorgname.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.memo.visible" :style="{}"  :sm="{ span: 24, offset: 0 }" :md="{ span: 24, offset: 0 }" :lg="{ span: 24, offset: 0 }" :xl="{ span: 24, offset: 0 }">
    <app-form-item name='memo' :itemRules="this.rules.memo" class='' :caption="$t('entities.socaccount.main_form.details.memo')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.memo.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.memo" :autoSize="{minRows: 10}"  :disabled="detailsModel.memo.disabled" type='textarea' style="height:100px;" ></input-box>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfmajortext: [
            { type: 'string', message: '参保账户名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '参保账户名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '参保账户名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '参保账户名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        socaccountname: [
            { type: 'string', message: '参保账户名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '参保账户名称 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '参保账户名称 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '参保账户名称 值不能为空', trigger: 'blur' },
        ],
        ormorgname: [
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '参保账户基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.socaccount.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfkey: new FormItemModel({ caption: '参保账户标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '参保账户名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        socaccountname: new FormItemModel({ caption: '参保账户名称', detailType: 'FORMITEM', name: 'socaccountname', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.socaccount){
            Object.assign(this.context,{socaccount:data.socaccount})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.socaccount = copyData.srfkey;
        copyData.socaccountid = copyData.srfkey;
        Object.assign(this.context,{socaccount:copyData.socaccount})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'socaccount_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'SocAccountEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SocAccountEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'SocAccountEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'SocAccountEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.socaccount || Object.is(arg.socaccount, '')) {
            return;
        }
