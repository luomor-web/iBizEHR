...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ehr-app-ehrpar\/dynadashboards\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ehr-app-ehrpar\/dynadashboards\/process-definitions-nodes$/)).reply((config: any) => {
...
// Select
mock.onGet(new RegExp(/^\/dynadashboards\/([a-zA-Z0-9\-\;]{1,35})\/select$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynadashboard 方法: Select");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const matchArray:any = new RegExp(/^\/dynadashboards\/([a-zA-Z0-9\-\;]{1,35})\/select$/).exec(config.url);
    let tempValue: any = {};
...
// Update
mock.onPut(new RegExp(/^\/dynadashboards\/?([a-zA-Z0-9\-\;]{0,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynadashboard 方法: Update");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
mock.onPost(new RegExp(/^\/dynadashboards\/?([a-zA-Z0-9\-\;]{0,35})\/checkkey$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynadashboard 方法: CheckKey");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['dynadashboardid'];
    const matchArray:any = new RegExp(/^\/dynadashboards\/([a-zA-Z0-9\-\;]{1,35})\/checkkey$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FetchDefault
mock.onGet(new RegExp(/^\/dynadashboards\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynadashboard 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDefault
mock.onGet(new RegExp(/^\/dynadashboards\/fetchdefault(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:dynadashboard 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
