<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='parkhzcmx_main' style="">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        szqz: [
            { type: 'number', message: '考核权重（%） 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '考核权重（%） 值必须为数值类型', trigger: 'blur' },
...
, 
     ...
     */
    @Watch('data.szqz')
    onSzqzChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'szqz', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.parkhzcmx){
            Object.assign(this.context,{parkhzcmx:data.parkhzcmx})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.parkhzcmx = copyData.srfkey;
        copyData.parkhzcmxid = copyData.srfkey;
        Object.assign(this.context,{parkhzcmx:copyData.parkhzcmx})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'parkhzcmx_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ParKhzcmxEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParKhzcmxEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ParKhzcmxEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ParKhzcmxEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.parkhzcmx || Object.is(arg.parkhzcmx, '')) {
            return;
        }
