<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='parldkhqz_qzfp' style="">
...
<i-col v-show="detailsModel.khdx.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    ...
    <app-span   name='khdx'
:value="data.khdx" tag='EhrCodeList0015' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、"   :data="data"
  :context="context"
  :viewparams="viewparams"
  :itemParam="{}" 
style=""></app-span>
...
<i-col v-show="detailsModel.pfzt.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    ...
    :viewparams="viewparams"
    :itemParam="{}" 
    :disabled="detailsModel.pfzt.disabled"  
    tag='EhrCodeList0209' 
    codelistType='STATIC'
...
export default class QZFPBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfkey: [
            { type: 'string', message: '评分规则标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '评分规则标识 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '评分规则标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '评分规则标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '评分规则 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '评分规则 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '评分规则 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '评分规则 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        pfzt: [
            ...
            { type: 'string', message: '评分主体 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '评分主体 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '评分主体 值不能为空', trigger: 'blur' },
        ],
        qzzb: [
            { type: 'number', message: '权重占比（%） 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '权重占比（%） 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '权重占比（%） 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '权重占比（%） 值不能为空', trigger: 'blur' },
        ],
        parldkhqzid: [
            { type: 'string', message: '评分规则标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '评分规则标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        srfkey: new FormItemModel({ caption: '评分规则标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '评分规则', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        qzzb: new FormItemModel({ caption: '权重占比（%）', detailType: 'FORMITEM', name: 'qzzb', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        parldkhqzid: new FormItemModel({ caption: '评分规则标识', detailType: 'FORMITEM', name: 'parldkhqzid', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    onPfztChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'pfzt', newVal: newVal, oldVal: oldVal });
    }
    ...
    onQzzbChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'qzzb', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 parldkhqzid 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof QZFP
    ...
    onParldkhqzidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'parldkhqzid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.parldkhqz){
            Object.assign(this.context,{parldkhqz:data.parldkhqz})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.parldkhqz = copyData.srfkey;
        copyData.parldkhqzid = copyData.srfkey;
        Object.assign(this.context,{parldkhqz:copyData.parldkhqz})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'parldkhqz_qzfp',popTitle:'权重分配'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdkhqzEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdkhqzEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ParLdkhqzEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ParLdkhqzEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.parldkhqz || Object.is(arg.parldkhqz, '')) {
            return;
        }
