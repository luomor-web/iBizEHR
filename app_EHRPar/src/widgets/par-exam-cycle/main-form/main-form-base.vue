<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='parexamcycle_main' style="">
...
<i-col v-show="detailsModel.parexamcyclename.visible" :style="{}"  :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
    <app-form-item name='parexamcyclename' :itemRules="this.rules.parexamcyclename" class='' :caption="$t('entities.parexamcycle.main_form.details.parexamcyclename')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.parexamcyclename.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.parexamcyclename"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.parexamcyclename.disabled" type='text'  style=""></input-box>
</app-form-item>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfkey: [
            { type: 'string', message: '考核周期标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考核周期标识 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考核周期标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考核周期标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '考核周期 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考核周期 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考核周期 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考核周期 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        parexamcycleid: [
            ...
            { required: false, type: 'string', message: '周期标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '周期标识 值不能为空', trigger: 'blur' },
        ],
        parexamcyclename: [
            { type: 'string', message: '考核周期 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考核周期 值必须为字符串类型', trigger: 'blur' },
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '考核周期', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'entities.parexamcycle.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfkey: new FormItemModel({ caption: '考核周期标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '考核周期', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        parexamcyclename: new FormItemModel({ caption: '考核周期', detailType: 'FORMITEM', name: 'parexamcyclename', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof MainBase
     */
    @Watch('data.parexamcycleid')
    onParexamcycleidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'parexamcycleid', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 parexamcyclename 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.parexamcyclename')
    onParexamcyclenameChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'parexamcyclename', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.parexamcycle){
            Object.assign(this.context,{parexamcycle:data.parexamcycle})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.parexamcycle = copyData.srfkey;
        copyData.parexamcycleid = copyData.srfkey;
        Object.assign(this.context,{parexamcycle:copyData.parexamcycle})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'parexamcycle_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ParExamCycleEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParExamCycleEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ParExamCycleEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ParExamCycleEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.parexamcycle || Object.is(arg.parexamcycle, '')) {
            return;
        }
