  ...
  <row>
    <i-col span="20">
      <row>
              ...
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pimarchivesloanandreturn.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_zt_eq.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_zt_eq' :itemRules="this.rules.n_zt_eq" class='' :caption="$t('entities.pimarchivesloanandreturn.default_searchform.details.n_zt_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_zt_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
               <dropdown-list v-model="data.n_zt_eq" :data="data" :itemParam="{}" :disabled="detailsModel.n_zt_eq.disabled"  tag='PIMCL_DAGHZT' codelistType='STATIC' placeholder='请选择...' style=""></dropdown-list>
          </app-form-item>
...
, 
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOANANDRETURNGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOANANDRETURNGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
