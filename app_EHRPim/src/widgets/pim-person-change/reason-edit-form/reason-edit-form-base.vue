<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pimpersonchange_reasonedit' style="">
...
<i-col v-show="detailsModel.bgnr.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='bgnr' :itemRules="this.rules.bgnr" class='' :caption="$t('entities.pimpersonchange.reasonedit_form.details.bgnr')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.bgnr.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.bgnr"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.bgnr.disabled" type='text'  style=""></input-box>
</app-form-item>
...
export default class ReasonEditBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        bgnr: [
            ...
            { required: false, type: 'string', message: '变更内容 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '变更内容 值不能为空', trigger: 'blur' },
        ],
        reason: [
            { type: 'string', message: '拒绝原因 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '拒绝原因 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '拒绝原因 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '拒绝原因 值不能为空', trigger: 'blur' },
        ],
        pimpersonchangeid: [
            { type: 'string', message: '人员信息变更审核标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '人员信息变更审核标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        bglx: new FormItemModel({ caption: '变更类型', detailType: 'FORMITEM', name: 'bglx', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
        bgnr: new FormItemModel({ caption: '变更内容', detailType: 'FORMITEM', name: 'bgnr', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
        reason: new FormItemModel({ caption: '拒绝原因', detailType: 'FORMITEM', name: 'reason', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pimpersonchange_reasonedit',popTitle:'驳回原因'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEReasonEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEReasonEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEReasonEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEReasonEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
