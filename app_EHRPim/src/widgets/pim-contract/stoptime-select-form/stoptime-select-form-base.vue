<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pimcontract_stoptimeselect' style="">
...
<i-col v-show="detailsModel.jsrq.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='jsrq' :itemRules="this.rules.jsrq" class='' :caption="$t('entities.pimcontract.stoptimeselect_form.details.jsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class StoptimeSelectBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        htsyts: [
            { type: 'number', message: '合同剩余天数 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '合同剩余天数 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '合同剩余天数 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '合同剩余天数 值不能为空', trigger: 'blur' },
        ],
        htsyts_color: [
            { type: 'number', message: '合同剩余天数颜色 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '合同剩余天数颜色 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '合同剩余天数颜色 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '合同剩余天数颜色 值不能为空', trigger: 'blur' },
        ],
        pimcontractid: [
            { type: 'string', message: '合同信息标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '合同信息标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        htbh: new FormItemModel({ caption: '合同编号', detailType: 'FORMITEM', name: 'htbh', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
...
, 
        htsyts: new FormItemModel({ caption: '合同剩余天数', detailType: 'FORMITEM', name: 'htsyts', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        htsyts_color: new FormItemModel({ caption: '合同剩余天数颜色', detailType: 'FORMITEM', name: 'htsyts_color', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    onHtsytsChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'htsyts', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 htsyts_color 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof StoptimeSelectBase
    ...
    onHtsyts_colorChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'htsyts_color', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pimcontract_stoptimeselect',popTitle:'合同终止时间选择'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTStopTimeSelectEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTStopTimeSelectEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTStopTimeSelectEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTStopTimeSelectEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    */
    ...
    public createDefault(){                    
        ...
        if (this.data.hasOwnProperty('htsyts_color')) {
            this.data['htsyts_color'] = null;
        }
    }
...
<style lang='less'>
@import './stoptime-select-form.less';
</style>