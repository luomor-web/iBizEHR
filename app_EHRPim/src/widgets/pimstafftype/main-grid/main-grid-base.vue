...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('sbm')">
                <el-table-column show-overflow-tooltip :prop="'sbm'" :label="$t('entities.pimstafftype.main_grid.columns.sbm')" :width="150" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('pimstafftypename')">
                <el-table-column show-overflow-tooltip :prop="'pimstafftypename'" :label="$t('entities.pimstafftype.main_grid.columns.pimstafftypename')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='pimstafftypename' editorType="TEXTBOX" :value="row.pimstafftypename"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('shgz')">
                <el-table-column show-overflow-tooltip :prop="'shgz'" :label="$t('entities.pimstafftype.main_grid.columns.shgz')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PIMCL_STAFFTYPE' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.shgz" tag='PIMCL_STAFFTYPE' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected minorSortPSDEF: string = 'sbm';
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            name: 'sbm',
            label: '员工类型代码',
            langtag: 'entities.pimstafftype.main_grid.columns.sbm',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '员工类型',
            langtag: 'entities.pimstafftype.main_grid.columns.pimstafftypename',
            show: true,
            util: 'PX'
        },
        {
            name: 'shgz',
            label: '入职审核规则',
            langtag: 'entities.pimstafftype.main_grid.columns.shgz',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected rules: any = {
        shgz: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '入职审核规则 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '入职审核规则 值不能为空', trigger: 'blur' },
        ],
        pimstafftypename: [
        ...
        sbm: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工类型代码 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工类型代码 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工类型管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工类型管理标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMSTAFFTYPEGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMSTAFFTYPEGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimstafftypename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimstafftype: keys.join(';') }),Object.assign({ pimstafftype: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimstafftype,$event.pimstafftype);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('pimstafftype_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('pimstafftype_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PIMSTAFFTYPEGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PIMSTAFFTYPEGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pimstafftype){
                    Object.assign(this.context,{pimstafftype:item.pimstafftype})
                }
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMSTAFFTYPEGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimstafftype,args.row.pimstafftype);
        });
