...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected minorSortPSDEF: string = 'snumber';
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            name: 'snumber',
            label: '排序编码',
            langtag: 'entities.pcmrcxl.main_grid.columns.snumber',
            show: true,
            util: 'px'
        },
        {
            name: 'rcxllx',
            label: '人才序列类型',
            langtag: 'entities.pcmrcxl.main_grid.columns.rcxllx',
            show: true,
            util: 'px'
        },
        {
            name: 'rcxlmc',
            label: '人才序列名称',
            langtag: 'entities.pcmrcxl.main_grid.columns.rcxlmc',
            show: true,
            util: 'px'
        },
        {
            name: 'code',
            label: '序列号',
            langtag: 'entities.pcmrcxl.main_grid.columns.code',
            show: true,
            util: 'px'
        },
        {
            name: 'xldj',
            label: '序列等级',
            langtag: 'entities.pcmrcxl.main_grid.columns.xldj',
            show: true,
            util: 'px'
        },
        {
            name: 'pcmrcxlname2',
            label: '上级',
            langtag: 'entities.pcmrcxl.main_grid.columns.pcmrcxlname2',
            show: true,
            util: 'px'
        },
    ]
    ...
    protected rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '人才序列标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '人才序列标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.rcxlmc;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmrcxl: keys.join(';') }),Object.assign({ pcmrcxl: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmrcxl,$event.pcmrcxl);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('pcmrcxl_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('pcmrcxl_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pcmrcxl){
                    Object.assign(this.context,{pcmrcxl:item.pcmrcxl})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmrcxl,args.row.pcmrcxl);
        });
