<template>
  ...
  <row>
    <i-col span="20">
      <row>
                    <i-col v-show="detailsModel.n_rcxllx_eq.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_rcxllx_eq' :itemRules="this.rules.n_rcxllx_eq" class='' :caption="$t('entities.pcmrcxl.default_searchform.details.n_rcxllx_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_rcxllx_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
               <dropdown-list v-model="data.n_rcxllx_eq" :data="data" :itemParam="{}" :disabled="detailsModel.n_rcxllx_eq.disabled"  tag='PCMCL_RCXLLX' codelistType='STATIC' placeholder='请选择...' style=""></dropdown-list>
          </app-form-item>
          
          </i-col>
          <i-col v-show="detailsModel.n_pcmrcxlname_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_pcmrcxlname_like' :itemRules="this.rules.n_pcmrcxlname_like" class='' :caption="$t('entities.pcmrcxl.default_searchform.details.n_pcmrcxlname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pcmrcxlname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_pcmrcxlname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_pcmrcxlname_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
export default class DefaultBase extends Vue implements ControlInterface {
     ...
     * @memberof Default
     */
    protected appEntityService: PCMRCXLService = new PCMRCXLService({ $store: this.$store });
    ...
    protected rules: any = {
        ...
        n_pcmrcxlname_like: [
            { type: 'string', message: '人才序列名称(%) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '人才序列名称(%) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '人才序列名称(%) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '人才序列名称(%) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_rcxllx_eq: new FormItemModel({ caption: '人才序列类型(=)', detailType: 'FORMITEM', name: 'n_rcxllx_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_pcmrcxlname_like: new FormItemModel({ caption: '人才序列名称(%)', detailType: 'FORMITEM', name: 'n_pcmrcxlname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };

    /**
     * 监控表单属性 n_rcxllx_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_rcxllx_eq')
    onN_rcxllx_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_rcxllx_eq', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 n_pcmrcxlname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_pcmrcxlname_like')
    onN_pcmrcxlname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_pcmrcxlname_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMRCXLPickupGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
