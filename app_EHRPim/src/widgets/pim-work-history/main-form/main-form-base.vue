<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pimworkhistory_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pimworkhistory.main_form.details.group1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.pimpersonname.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='pimpersonname' :itemRules="this.rules.pimpersonname" class='' :caption="$t('entities.pimworkhistory.main_form.details.pimpersonname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.pimpersonname.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.qssj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='qssj' :itemRules="this.rules.qssj" class='' :caption="$t('entities.pimworkhistory.main_form.details.qssj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.qssj.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.jssj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='jssj' :itemRules="this.rules.jssj" class='' :caption="$t('entities.pimworkhistory.main_form.details.jssj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jssj.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.bz.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='bz' :itemRules="this.rules.bz" class='' :caption="$t('entities.pimworkhistory.main_form.details.bz')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.bz.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        qssj: [
            { type: 'string', message: '起始时间 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '起始时间 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '起始时间 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '起始时间 值不能为空', trigger: 'blur' },
        ],
        jssj: [
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '工作履历基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'entities.pimworkhistory.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        qssj: new FormItemModel({ caption: '起始时间', detailType: 'FORMITEM', name: 'qssj', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        pimworkhistoryid: new FormItemModel({ caption: '工作履历标识', detailType: 'FORMITEM', name: 'pimworkhistoryid', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    /**
     * 监控表单属性 pimworkhistoryid 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.pimworkhistoryid')
    onPimworkhistoryidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'pimworkhistoryid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.pimworkhistory){
            Object.assign(this.context,{pimworkhistory:data.pimworkhistory})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.pimworkhistory = copyData.srfkey;
        copyData.pimworkhistoryid = copyData.srfkey;
        Object.assign(this.context,{pimworkhistory:copyData.pimworkhistory})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pimworkhistory_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMWORKHISTORYEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMWORKHISTORYEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PIMWORKHISTORYEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PIMWORKHISTORYEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.pimworkhistory || Object.is(arg.pimworkhistory, '')) {
            return;
        }
