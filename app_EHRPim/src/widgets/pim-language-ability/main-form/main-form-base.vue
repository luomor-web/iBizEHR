<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pimlanguageability_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pimlanguageability.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
...
<i-col v-show="detailsModel.wyyz.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    ...
    <app-radio-group v-model="data.wyyz"  :disabled="detailsModel.wyyz.disabled"  name="wyyz" tag='PIMCL_YYZL' codelistType='DYNAMIC'  style=""></app-radio-group>
</app-form-item>
...
<i-col v-show="detailsModel.wydj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    ...
    <app-radio-group v-model="data.wydj"  :disabled="detailsModel.wydj.disabled"  name="wydj" tag='LanguageLevel' codelistType='DYNAMIC'  style=""></app-radio-group>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.wydjhqsj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='wydjhqsj' :itemRules="this.rules.wydjhqsj" class='' :caption="$t('entities.pimlanguageability.main_form.details.wydjhqsj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.wydjhqsj.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.bz.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='bz' :itemRules="this.rules.bz" class='' :caption="$t('entities.pimlanguageability.main_form.details.bz')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.bz.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.fj.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='fj' :itemRules="this.rules.fj" class='' :caption="$t('entities.pimlanguageability.main_form.details.fj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.fj.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '语言能力信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.pimlanguageability.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        wyyz: new FormItemModel({ caption: '语种', detailType: 'FORMITEM', name: 'wyyz', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.wyyz')
    onWyyzChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'wyyz', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pimlanguageability_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
