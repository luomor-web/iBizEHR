                ...
                <el-table-column show-overflow-tooltip :prop="'layerno'" :label="$t('entities.archivescenter.main_grid.columns.layerno')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PIMCL_LAYERNO' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            name: 'serialno',
            label: '排序号',
            langtag: 'entities.archivescenter.main_grid.columns.serialno',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '档案室名称',
            langtag: 'entities.archivescenter.main_grid.columns.archivescentername',
            show: true,
            util: 'PX'
        },
        {
            name: 'cabinetno',
            label: '柜号',
            langtag: 'entities.archivescenter.main_grid.columns.cabinetno',
            show: true,
            util: 'PX'
        },
        {
            name: 'layerno',
            label: '层号',
            langtag: 'entities.archivescenter.main_grid.columns.layerno',
            show: true,
            util: 'PX'
        },
        {
            name: 'bnumber',
            label: '编号',
            langtag: 'entities.archivescenter.main_grid.columns.bnumber',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ archivescenter: keys.join(';') }),Object.assign({ archivescenter: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected save(args: any[], params?: any, $event?: any, xData?: any): void {
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                Object.assign(item,{viewparams:this.viewparams});
                if(item.archivescenter){
                    Object.assign(this.context,{archivescenter:item.archivescenter})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.archivescenter,args.row.archivescenter);
        });
