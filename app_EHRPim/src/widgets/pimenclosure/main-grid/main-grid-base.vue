...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimenclosurename')">
                <el-table-column show-overflow-tooltip :prop="'pimenclosurename'" :label="$t('entities.pimenclosure.main_grid.columns.pimenclosurename')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.pimenclosurename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fjfl')">
                <el-table-column show-overflow-tooltip :prop="'fjfl'" :label="$t('entities.pimenclosure.main_grid.columns.fjfl')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.fjfl" tag='PIMCL_FJ' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.pimenclosure.main_grid.columns.updateman')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.pimenclosure.main_grid.columns.updatedate')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-format-data format="%1$tY-%1$tm-%1$td" :data="row.updatedate"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fjkz')">
                <el-table-column show-overflow-tooltip :prop="'fjkz'" :label="$t('entities.pimenclosure.main_grid.columns.fjkz')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.fjkz}}</span>
                    </template>
                </el-table-column>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            ...
            label: '附件信息名称',
            langtag: 'entities.pimenclosure.main_grid.columns.pimenclosurename',
            show: true,
            util: 'PX'
        },
        {
            name: 'fjfl',
            label: '附件分类',
            langtag: 'entities.pimenclosure.main_grid.columns.fjfl',
            show: true,
            util: 'px'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.pimenclosure.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.pimenclosure.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
        {
            name: 'fjkz',
            label: '附件快照',
            langtag: 'entities.pimenclosure.main_grid.columns.fjkz',
            show: true,
            util: 'px'
        },
    ]
    ...
    protected rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '附件信息标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '附件信息标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMENCLOSUREPimFujGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMENCLOSUREPimFujGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimenclosurename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimenclosure: keys.join(';') }),Object.assign({ pimenclosure: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimenclosure,$event.pimenclosure);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('pimenclosure_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('pimenclosure_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PIMENCLOSUREPimFujGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PIMENCLOSUREPimFujGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pimenclosure){
                    Object.assign(this.context,{pimenclosure:item.pimenclosure})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimenclosure,args.row.pimenclosure);
        });
