...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('serialnumber')">
                <el-table-column show-overflow-tooltip :prop="'serialnumber'" :label="$t('entities.archivalcatalogue.main_grid.columns.serialnumber')" :width="150" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('archivalcataloguename')">
                <el-table-column show-overflow-tooltip :prop="'archivalcataloguename'" :label="$t('entities.archivalcatalogue.main_grid.columns.archivalcataloguename')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            name: 'serialnumber',
            label: '排序号',
            langtag: 'entities.archivalcatalogue.main_grid.columns.serialnumber',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '档案目录名称',
            langtag: 'entities.archivalcatalogue.main_grid.columns.archivalcataloguename',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ archivalcatalogue: keys.join(';') }),Object.assign({ archivalcatalogue: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.archivalcatalogue){
                    Object.assign(this.context,{archivalcatalogue:item.archivalcatalogue})
                }
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.archivalcatalogue,args.row.archivalcatalogue);
        });
