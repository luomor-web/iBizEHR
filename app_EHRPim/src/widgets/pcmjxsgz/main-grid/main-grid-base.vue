...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('xh')">
                <el-table-column show-overflow-tooltip :prop="'xh'" :label="$t('entities.pcmjxsgz.main_grid.columns.xh')" :width="150" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('pcmjxsgzname')">
                <el-table-column show-overflow-tooltip :prop="'pcmjxsgzname'" :label="$t('entities.pcmjxsgz.main_grid.columns.pcmjxsgzname')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='pcmjxsgzname' editorType="TEXTBOX" :value="row.pcmjxsgzname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('duration')">
                <el-table-column show-overflow-tooltip :prop="'duration'" :label="$t('entities.pcmjxsgz.main_grid.columns.duration')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PIMCL_JXQ' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            name: 'xh',
            label: '排序号',
            langtag: 'entities.pcmjxsgz.main_grid.columns.xh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pcmjxsgzname',
            label: '见习生规则名称',
            langtag: 'entities.pcmjxsgz.main_grid.columns.pcmjxsgzname',
            show: true,
            util: 'PX'
        },
        {
            name: 'duration',
            label: '见习期限',
            langtag: 'entities.pcmjxsgz.main_grid.columns.duration',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected rules: any = {
        duration: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习期限 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习期限 值不能为空', trigger: 'blur' },
        ],
        xh: [
        ...
        pcmjxsgzname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生规则名称 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生规则名称 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生规则标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生规则标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMJXSGZGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMJXSGZGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmjxsgzname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmjxsgz: keys.join(';') }),Object.assign({ pcmjxsgz: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmjxsgz,$event.pcmjxsgz);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('pcmjxsgz_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('pcmjxsgz_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMJXSGZGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMJXSGZGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pcmjxsgz){
                    Object.assign(this.context,{pcmjxsgz:item.pcmjxsgz})
                }
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMJXSGZGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmjxsgz,args.row.pcmjxsgz);
        });
