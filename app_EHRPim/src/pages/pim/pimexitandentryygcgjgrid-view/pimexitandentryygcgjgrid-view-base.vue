  ...
  <app-layout viewName="pimexitandentryygcgjgridview" viewTitle="出入境管理实体表格视图（因公出国境）" :className="{ 'view-container': true, 'default-mode-view': true, 'degridview': true, 'pimexitandentryygcgjgrid-view': true }" layoutMode="VIEW" :isShowUserInfo="isDefaultView()" :openMode="openMode" @close-view="closeView($event)">
    ...
    <template slot="headerRight">
      <div class="view-header-right">
        <app-header-menus :toolbarModel="toolBarModels" @menu-click="toolbar_click($event)" mode="view" :openMode="openMode" :isEnableQuickSearch="true" searchPlaceholder="出入境管理名称" v-model="query" @search="onSearch($event)"/>
      </div>
    </template>
 ...
 tbitem13: { name: 'tbitem13', caption: '导出数据','isShowCaption':true,'isShowIcon':true, tooltip: '导出数据', iconcls: 'fa fa-file-excel-o', icon: '', disabled: false, type: 'DEUIACTION', visabled: true, dataaccaction: '', uiaction: { tag: 'ExportExcel', target: '' }, MaxRowCount: 5000, class: '' },
    ...
    public toolbar_click($event: any, $event2?: any) {
        if (Object.is($event.tag, 'tbitem1_newyg')) {
            this.toolbar_tbitem1_newyg_click(null, '', $event2);
        }
        if (Object.is($event.tag, 'tbitem1_ksccgj')) {
            this.toolbar_tbitem1_ksccgj_click(null, '', $event2);
        }
