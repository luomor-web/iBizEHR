...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pimcontract.grcontracteditgrid_form.details.group1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.htbh.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='htbh' :itemRules="this.rules.htbh" class='' :caption="$t('entities.pimcontract.grcontracteditgrid_form.details.htbh')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.htbh.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.qsrq.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='qsrq' :itemRules="this.rules.qsrq" class='' :caption="$t('entities.pimcontract.grcontracteditgrid_form.details.qsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.qsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.jsrq.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
    <app-form-item name='jsrq' :itemRules="this.rules.jsrq" class='' :caption="$t('entities.pimcontract.grcontracteditgrid_form.details.jsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class GRContractEditGridBase extends Vue implements ControlInterface {
...
, 
        htzt: new FormItemModel({ caption: '合同状态', detailType: 'FORMITEM', name: 'htzt', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
...
, 
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTGRContractEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTGRContractEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTGRContractEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PIMCONTRACTGRContractEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
...
<style lang='less'>
@import './grcontract-edit-grid-form.less';
</style>