import sallog_zh_CN from '@locale/lanres/entities/sal-log/sal-log_zh_CN';
import trmdepart_zh_CN from '@locale/lanres/entities/trm-depart/trm-depart_zh_CN';
import trmtrainplanterm_zh_CN from '@locale/lanres/entities/trm-train-planterm/trm-train-planterm_zh_CN';
import salschemeitem_zh_CN from '@locale/lanres/entities/sal-scheme-item/sal-scheme-item_zh_CN';
import trmlgbcost_zh_CN from '@locale/lanres/entities/trm-lgb-cost/trm-lgb-cost_zh_CN';
import pcmjxsygzzsq_zh_CN from '@locale/lanres/entities/pcm-jxsygzzsq/pcm-jxsygzzsq_zh_CN';
import pcmrecruitment_zh_CN from '@locale/lanres/entities/pcm-recruitment/pcm-recruitment_zh_CN';
import pimcorrectionapply_zh_CN from '@locale/lanres/entities/pim-correction-apply/pim-correction-apply_zh_CN';
import ormorg_zh_CN from '@locale/lanres/entities/orm-org/orm-org_zh_CN';
import pcmnotice_zh_CN from '@locale/lanres/entities/pcm-notice/pcm-notice_zh_CN';
import salstdgwgzrate_zh_CN from '@locale/lanres/entities/sal-std-gw-gzrate/sal-std-gw-gzrate_zh_CN';
import pcmydmx_zh_CN from '@locale/lanres/entities/pcm-ydmx/pcm-ydmx_zh_CN';
import pimqualtype_zh_CN from '@locale/lanres/entities/pim-qual-type/pim-qual-type_zh_CN';
import pimmajorsetype_zh_CN from '@locale/lanres/entities/pim-major-setype/pim-major-setype_zh_CN';
import pcmwzd0001_zh_CN from '@locale/lanres/entities/pcm-wzd0001/pcm-wzd0001_zh_CN';
import pimtitle_zh_CN from '@locale/lanres/entities/pim-title/pim-title_zh_CN';
import parlhmbkhmx_zh_CN from '@locale/lanres/entities/par-lhmbkhmx/par-lhmbkhmx_zh_CN';
import attendencesetup_zh_CN from '@locale/lanres/entities/att-endence-setup/att-endence-setup_zh_CN';
import pcmddsqdmx_zh_CN from '@locale/lanres/entities/pcm-ddsqdmx/pcm-ddsqdmx_zh_CN';
import pcmschooloffice_zh_CN from '@locale/lanres/entities/pcm-school-office/pcm-school-office_zh_CN';
import pimtypecontract_zh_CN from '@locale/lanres/entities/pim-type-contract/pim-type-contract_zh_CN';
import trmteachercharge_zh_CN from '@locale/lanres/entities/trm-teacher-charge/trm-teacher-charge_zh_CN';
import salstdzc_zh_CN from '@locale/lanres/entities/sal-stdzc/sal-stdzc_zh_CN';
import pcmxygzzsq_zh_CN from '@locale/lanres/entities/pcm-xygzzsq/pcm-xygzzsq_zh_CN';
import vacworkday_zh_CN from '@locale/lanres/entities/vac-work-day/vac-work-day_zh_CN';
import attendencetype_zh_CN from '@locale/lanres/entities/att-endence-type/att-endence-type_zh_CN';
import pimoutput_zh_CN from '@locale/lanres/entities/pim-output/pim-output_zh_CN';
import parjxzgpc_zh_CN from '@locale/lanres/entities/par-jxzgpc/par-jxzgpc_zh_CN';
import vacsystemhistory_zh_CN from '@locale/lanres/entities/vac-system-history/vac-system-history_zh_CN';
import attendancemreportmx_zh_CN from '@locale/lanres/entities/att-endance-mreportmx/att-endance-mreportmx_zh_CN';
import trmteachertrain_zh_CN from '@locale/lanres/entities/trm-teacher-train/trm-teacher-train_zh_CN';
import parjxkhjcsz_zh_CN from '@locale/lanres/entities/par-jxkhjcsz/par-jxkhjcsz_zh_CN';
import trmtrainperson_zh_CN from '@locale/lanres/entities/trm-train-person/trm-train-person_zh_CN';
import pimarchivesloanandreturn_zh_CN from '@locale/lanres/entities/pim-archive-sloanandreturn/pim-archive-sloanandreturn_zh_CN';
import pcmzjdyzw_zh_CN from '@locale/lanres/entities/pcm-zjdyzw/pcm-zjdyzw_zh_CN';
import pcmrcxl_zh_CN from '@locale/lanres/entities/pcm-rcxl/pcm-rcxl_zh_CN';
import pimbyzzjlmx_zh_CN from '@locale/lanres/entities/pim-byzzjlmx/pim-byzzjlmx_zh_CN';
import ormbmgwbz_zh_CN from '@locale/lanres/entities/orm-bmgwbz/orm-bmgwbz_zh_CN';
import pimcontract_zh_CN from '@locale/lanres/entities/pim-contract/pim-contract_zh_CN';
import saltype_zh_CN from '@locale/lanres/entities/sal-type/sal-type_zh_CN';
import pcmprofileinitnumber_zh_CN from '@locale/lanres/entities/pcm-profile-init-number/pcm-profile-init-number_zh_CN';
import pcmworkresume_zh_CN from '@locale/lanres/entities/pcm-work-resume/pcm-work-resume_zh_CN';
import trmtrainactment_zh_CN from '@locale/lanres/entities/trm-train-actment/trm-train-actment_zh_CN';
import pimsocialsecurity_zh_CN from '@locale/lanres/entities/pim-social-security/pim-social-security_zh_CN';
import pimworkhistory_zh_CN from '@locale/lanres/entities/pim-work-history/pim-work-history_zh_CN';
import ormorgdz_zh_CN from '@locale/lanres/entities/orm-orgdz/orm-orgdz_zh_CN';
import parlhmbmx_zh_CN from '@locale/lanres/entities/par-lhmbmx/par-lhmbmx_zh_CN';
import pimpersonability_zh_CN from '@locale/lanres/entities/pim-person-ability/pim-person-ability_zh_CN';
import trminvoice_zh_CN from '@locale/lanres/entities/trm-invoice/trm-invoice_zh_CN';
import ormtitle_zh_CN from '@locale/lanres/entities/orm-title/orm-title_zh_CN';
import vacleacetype_zh_CN from '@locale/lanres/entities/vac-leace-type/vac-leace-type_zh_CN';
import salstdgw_zh_CN from '@locale/lanres/entities/sal-stdgw/sal-stdgw_zh_CN';
import pimarchiveschange_zh_CN from '@locale/lanres/entities/pim-archives-change/pim-archives-change_zh_CN';
import pcmdetail_zh_CN from '@locale/lanres/entities/pcm-detail/pcm-detail_zh_CN';
import pimexpaccount_zh_CN from '@locale/lanres/entities/pim-expaccount/pim-expaccount_zh_CN';
import pcmsyqgz_zh_CN from '@locale/lanres/entities/pcm-syqgz/pcm-syqgz_zh_CN';
import vacholiday_zh_CN from '@locale/lanres/entities/vac-holiday/vac-holiday_zh_CN';
import salsalarydetail_zh_CN from '@locale/lanres/entities/sal-salary-detail/sal-salary-detail_zh_CN';
import parjxmtfk_zh_CN from '@locale/lanres/entities/par-jxmtfk/par-jxmtfk_zh_CN';
import attendancemreport_zh_CN from '@locale/lanres/entities/att-endance-mreport/att-endance-mreport_zh_CN';
import trmtrainactapply_zh_CN from '@locale/lanres/entities/trm-train-actapply/trm-train-actapply_zh_CN';
import pcmgxml_zh_CN from '@locale/lanres/entities/pcm-gxml/pcm-gxml_zh_CN';
import pcmtxbz_zh_CN from '@locale/lanres/entities/pcm-txbz/pcm-txbz_zh_CN';
import ormxmxqhz_zh_CN from '@locale/lanres/entities/orm-xmxqhz/orm-xmxqhz_zh_CN';
import pimqualmajor_zh_CN from '@locale/lanres/entities/pim-qual-major/pim-qual-major_zh_CN';
import pcmxygzzkhjgjl_zh_CN from '@locale/lanres/entities/pcm-xygzzkhjgjl/pcm-xygzzkhjgjl_zh_CN';
import parjxkhmbmx_zh_CN from '@locale/lanres/entities/par-jxkhmbmx/par-jxkhmbmx_zh_CN';
import vacleavemanage_zh_CN from '@locale/lanres/entities/vac-leave-manage/vac-leave-manage_zh_CN';
import pcmlog_zh_CN from '@locale/lanres/entities/pcm-log/pcm-log_zh_CN';
import pimvacation_zh_CN from '@locale/lanres/entities/pim-vacation/pim-vacation_zh_CN';
import pimexitandentry_zh_CN from '@locale/lanres/entities/pim-exitandentry/pim-exitandentry_zh_CN';
import trmplanformu_zh_CN from '@locale/lanres/entities/trm-plan-formu/trm-plan-formu_zh_CN';
import trmtraindemand_zh_CN from '@locale/lanres/entities/trm-train-demand/trm-train-demand_zh_CN';
import pimpersonchange_zh_CN from '@locale/lanres/entities/pim-person-change/pim-person-change_zh_CN';
import pcmeducationexperience_zh_CN from '@locale/lanres/entities/pcm-education-experience/pcm-education-experience_zh_CN';
import trmtrainplan_zh_CN from '@locale/lanres/entities/trm-train-plan/trm-train-plan_zh_CN';
import pimachievements_zh_CN from '@locale/lanres/entities/pim-achievements/pim-achievements_zh_CN';
import pcmxkml_zh_CN from '@locale/lanres/entities/pcm-xkml/pcm-xkml_zh_CN';
import pimlanguageability_zh_CN from '@locale/lanres/entities/pim-language-ability/pim-language-ability_zh_CN';
import pimperson_zh_CN from '@locale/lanres/entities/pim-person/pim-person_zh_CN';
import trmtraffic_zh_CN from '@locale/lanres/entities/trm-traffic/trm-traffic_zh_CN';
import pimexaminationresults_zh_CN from '@locale/lanres/entities/pim-examination-results/pim-examination-results_zh_CN';
import pimarchives_zh_CN from '@locale/lanres/entities/pim-archives/pim-archives_zh_CN';
import vacleavesystemdtail_zh_CN from '@locale/lanres/entities/vac-leave-system-dtail/vac-leave-system-dtail_zh_CN';
import parjxzgpcmx_zh_CN from '@locale/lanres/entities/par-jxzgpcmx/par-jxzgpcmx_zh_CN';
import ormrank_zh_CN from '@locale/lanres/entities/orm-rank/orm-rank_zh_CN';
import vacleavedetail_zh_CN from '@locale/lanres/entities/vac-leave-detail/vac-leave-detail_zh_CN';
import trmtraindata_zh_CN from '@locale/lanres/entities/trm-train-data/trm-train-data_zh_CN';
import pcmgbhmc_zh_CN from '@locale/lanres/entities/pcm-gbhmc/pcm-gbhmc_zh_CN';
import parjgbmpjbz_zh_CN from '@locale/lanres/entities/par-jgbmpjbz/par-jgbmpjbz_zh_CN';
import parndlhmbkh_zh_CN from '@locale/lanres/entities/par-ndlhmbkh/par-ndlhmbkh_zh_CN';
import pcmydjzmx_zh_CN from '@locale/lanres/entities/pcm-ydjzmx/pcm-ydjzmx_zh_CN';
import pcmtestresult_zh_CN from '@locale/lanres/entities/pcm-test-result/pcm-test-result_zh_CN';
import ormorginfo_zh_CN from '@locale/lanres/entities/orm-org-info/orm-org-info_zh_CN';
import vacinitnxj_zh_CN from '@locale/lanres/entities/vac-initnxj/vac-initnxj_zh_CN';
import salstdzx_zh_CN from '@locale/lanres/entities/sal-stdzx/sal-stdzx_zh_CN';
import pimworkflow_zh_CN from '@locale/lanres/entities/pim-workflow/pim-workflow_zh_CN';
import salitem_zh_CN from '@locale/lanres/entities/sal-item/sal-item_zh_CN';
import trmagencyrecord_zh_CN from '@locale/lanres/entities/trm-agency-record/trm-agency-record_zh_CN';
import ormbmkqdz_zh_CN from '@locale/lanres/entities/orm-bmkqdz/orm-bmkqdz_zh_CN';
import pcmpcmsgqmgr_zh_CN from '@locale/lanres/entities/pcm-pcm-sgq-mgr/pcm-pcm-sgq-mgr_zh_CN';
import trmdepartneeds_zh_CN from '@locale/lanres/entities/trm-depart-needs/trm-depart-needs_zh_CN';
import salstdxmgz_zh_CN from '@locale/lanres/entities/sal-std-xmgz/sal-std-xmgz_zh_CN';
import pcmcsrcmspj_zh_CN from '@locale/lanres/entities/pcm-csrcmspj/pcm-csrcmspj_zh_CN';
import parkhzcmx_zh_CN from '@locale/lanres/entities/par-khzcmx/par-khzcmx_zh_CN';
import salstdzyzg_zh_CN from '@locale/lanres/entities/sal-stdzyzg/sal-stdzyzg_zh_CN';
import pimarchivesrecord_zh_CN from '@locale/lanres/entities/pim-archives-record/pim-archives-record_zh_CN';
import pcmreason_zh_CN from '@locale/lanres/entities/pcm-reason/pcm-reason_zh_CN';
import ormorgsector_zh_CN from '@locale/lanres/entities/orm-orgsector/orm-orgsector_zh_CN';
import trmdemdeftion_zh_CN from '@locale/lanres/entities/trm-demdeftion/trm-demdeftion_zh_CN';
import parjxjg_zh_CN from '@locale/lanres/entities/par-jxjg/par-jxjg_zh_CN';
import salsubject_zh_CN from '@locale/lanres/entities/sal-subject/sal-subject_zh_CN';
import pcmydlzmx_zh_CN from '@locale/lanres/entities/pcm-ydlzmx/pcm-ydlzmx_zh_CN';
import pimtitlecatalogue_zh_CN from '@locale/lanres/entities/pim-title-catalogue/pim-title-catalogue_zh_CN';
import parxmbpjbz_zh_CN from '@locale/lanres/entities/par-xmbpjbz/par-xmbpjbz_zh_CN';
import vacleavesystem_zh_CN from '@locale/lanres/entities/vac-leave-system/vac-leave-system_zh_CN';
import pcmypzjkxx_zh_CN from '@locale/lanres/entities/pcm-ypzjkxx/pcm-ypzjkxx_zh_CN';
import salstdgl_zh_CN from '@locale/lanres/entities/sal-stdgl/sal-stdgl_zh_CN';
import pimarmycadres_zh_CN from '@locale/lanres/entities/pim-armycadres/pim-armycadres_zh_CN';
import pimbyygzzsq_zh_CN from '@locale/lanres/entities/pim-byygzzsq/pim-byygzzsq_zh_CN';
import trmtrainteacher_zh_CN from '@locale/lanres/entities/trm-train-teacher/trm-train-teacher_zh_CN';
import pcmjxsbdjl_zh_CN from '@locale/lanres/entities/pcm-jxsbdjl/pcm-jxsbdjl_zh_CN';
import trmtrainbuapply_zh_CN from '@locale/lanres/entities/trm-train-buapply/trm-train-buapply_zh_CN';
import codelist1_zh_CN from '@locale/lanres/entities/code-list1/code-list1_zh_CN';
import trmtrainagency_zh_CN from '@locale/lanres/entities/trm-train-agency/trm-train-agency_zh_CN';
import pimresearchfindings_zh_CN from '@locale/lanres/entities/pim-research-findings/pim-research-findings_zh_CN';
import salscheme_zh_CN from '@locale/lanres/entities/sal-scheme/sal-scheme_zh_CN';
import pimlwgz_zh_CN from '@locale/lanres/entities/pim-lwgz/pim-lwgz_zh_CN';
import pimcontracttype_zh_CN from '@locale/lanres/entities/pim-contract-type/pim-contract-type_zh_CN';
import salsalary_zh_CN from '@locale/lanres/entities/sal-salary/sal-salary_zh_CN';
import pcmnorecyclinglibrary_zh_CN from '@locale/lanres/entities/pcm-no-recycling-library/pcm-no-recycling-library_zh_CN';
import attendancedate_zh_CN from '@locale/lanres/entities/att-endance-date/att-endance-date_zh_CN';
import pimnationmgr_zh_CN from '@locale/lanres/entities/pim-nation-mgr/pim-nation-mgr_zh_CN';
import ormxmrypzxq_zh_CN from '@locale/lanres/entities/orm-xmrypzxq/orm-xmrypzxq_zh_CN';
import pcmydjpmx_zh_CN from '@locale/lanres/entities/pcm-ydjpmx/pcm-ydjpmx_zh_CN';
import pimdistirbution_zh_CN from '@locale/lanres/entities/pim-distirbution/pim-distirbution_zh_CN';
import pimlabourcampany_zh_CN from '@locale/lanres/entities/pim-labourcampany/pim-labourcampany_zh_CN';
import pcmydjdmx_zh_CN from '@locale/lanres/entities/pcm-ydjdmx/pcm-ydjdmx_zh_CN';
import pcmtdzwxx_zh_CN from '@locale/lanres/entities/pcm-tdzwxx/pcm-tdzwxx_zh_CN';
import pimcontractsignorg_zh_CN from '@locale/lanres/entities/pim-contract-sign-org/pim-contract-sign-org_zh_CN';
import parjxkhmb_zh_CN from '@locale/lanres/entities/par-jxkhmb/par-jxkhmb_zh_CN';
import salpersonstd_zh_CN from '@locale/lanres/entities/sal-person-std/sal-person-std_zh_CN';
import trmtraindemandsum_zh_CN from '@locale/lanres/entities/trm-train-demand-sum/trm-train-demand-sum_zh_CN';
import salsalarybill_zh_CN from '@locale/lanres/entities/sal-salary-bill/sal-salary-bill_zh_CN';
import parznbmmx_zh_CN from '@locale/lanres/entities/par-znbmmx/par-znbmmx_zh_CN';
import trmemployeeretion_zh_CN from '@locale/lanres/entities/trm-employee-retion/trm-employee-retion_zh_CN';
import parjxndkhjg_zh_CN from '@locale/lanres/entities/par-jxndkhjg/par-jxndkhjg_zh_CN';
import trmstaff_zh_CN from '@locale/lanres/entities/trm-staff/trm-staff_zh_CN';
import ormxmbmx_zh_CN from '@locale/lanres/entities/orm-xmbmx/orm-xmbmx_zh_CN';
import parldkhqz_zh_CN from '@locale/lanres/entities/par-ldkhqz/par-ldkhqz_zh_CN';
import pimquestions_zh_CN from '@locale/lanres/entities/pim-questions/pim-questions_zh_CN';
import salruledetail_zh_CN from '@locale/lanres/entities/sal-rule-detail/sal-rule-detail_zh_CN';
import salpersonstddetail_zh_CN from '@locale/lanres/entities/salpersonstddetail/salpersonstddetail_zh_CN';
import trmstay_zh_CN from '@locale/lanres/entities/trm-stay/trm-stay_zh_CN';
import trmcoursesystem_zh_CN from '@locale/lanres/entities/trm-course-system/trm-course-system_zh_CN';
import attensummary_zh_CN from '@locale/lanres/entities/att-ensummary/att-ensummary_zh_CN';
import salrule_zh_CN from '@locale/lanres/entities/sal-rule/sal-rule_zh_CN';
import ormxmrlcb_zh_CN from '@locale/lanres/entities/orm-xmrlcb/orm-xmrlcb_zh_CN';
import personstatemgr_zh_CN from '@locale/lanres/entities/person-state-mgr/person-state-mgr_zh_CN';
import ormpostdetails_zh_CN from '@locale/lanres/entities/orm-post-details/orm-post-details_zh_CN';
import pimstafftype_zh_CN from '@locale/lanres/entities/pim-stafftype/pim-stafftype_zh_CN';
import pcmsgqmgr_zh_CN from '@locale/lanres/entities/pcm-sgq-mgr/pcm-sgq-mgr_zh_CN';
import salitemsub_zh_CN from '@locale/lanres/entities/sal-item-sub/sal-item-sub_zh_CN';
import pimgwtype_zh_CN from '@locale/lanres/entities/pim-gw-type/pim-gw-type_zh_CN';
import ormpostlib_zh_CN from '@locale/lanres/entities/orm-post-lib/orm-post-lib_zh_CN';
import pimarchivescenter_zh_CN from '@locale/lanres/entities/pim-archives-center/pim-archives-center_zh_CN';
import parjxbzgl_zh_CN from '@locale/lanres/entities/par-jxbzgl/par-jxbzgl_zh_CN';
import vacusenxjmx_zh_CN from '@locale/lanres/entities/vac-use-nxjmx/vac-use-nxjmx_zh_CN';
import vacsynjcx_zh_CN from '@locale/lanres/entities/vac-synjcx/vac-synjcx_zh_CN';
import pcmzpmegl_zh_CN from '@locale/lanres/entities/pcm-zpmegl/pcm-zpmegl_zh_CN';
import attensummarymx_zh_CN from '@locale/lanres/entities/att-ensummarymx/att-ensummarymx_zh_CN';
import trmhmatser_zh_CN from '@locale/lanres/entities/trm-hmatser/trm-hmatser_zh_CN';
import pcmtxfpsq_zh_CN from '@locale/lanres/entities/pcm-txfpsq/pcm-txfpsq_zh_CN';
import ormzwdqgz_zh_CN from '@locale/lanres/entities/orm-zwdqgz/orm-zwdqgz_zh_CN';
import pcmyddgmx_zh_CN from '@locale/lanres/entities/pcm-yddgmx/pcm-yddgmx_zh_CN';
import ormxmbq_zh_CN from '@locale/lanres/entities/orm-xmbq/orm-xmbq_zh_CN';
import attendancerecordtemp_zh_CN from '@locale/lanres/entities/att-endance-record-temp/att-endance-record-temp_zh_CN';
import pcmgxxktemp_zh_CN from '@locale/lanres/entities/pcmgxxk-temp/pcmgxxk-temp_zh_CN';
import pcmrzgl_zh_CN from '@locale/lanres/entities/pcm-rzgl/pcm-rzgl_zh_CN';
import trmtrainfillin_zh_CN from '@locale/lanres/entities/trm-train-fillin/trm-train-fillin_zh_CN';
import salstdxmgzrate_zh_CN from '@locale/lanres/entities/sal-std-xmgz-rate/sal-std-xmgz-rate_zh_CN';
import vacdayoff_zh_CN from '@locale/lanres/entities/vac-day-off/vac-day-off_zh_CN';
import trmstaffnodes_zh_CN from '@locale/lanres/entities/trm-staffnodes/trm-staffnodes_zh_CN';
import pcmawardswons_zh_CN from '@locale/lanres/entities/pcm-awards-wons/pcm-awards-wons_zh_CN';
import trmcouarrange_zh_CN from '@locale/lanres/entities/trm-couarrange/trm-couarrange_zh_CN';
import pimrewardpunishment_zh_CN from '@locale/lanres/entities/pim-reward-punishment/pim-reward-punishment_zh_CN';
import salstdzj_zh_CN from '@locale/lanres/entities/sal-stdzj/sal-stdzj_zh_CN';
import attendancerecord_zh_CN from '@locale/lanres/entities/att-endance-record/att-endance-record_zh_CN';
import pimpatent_zh_CN from '@locale/lanres/entities/pim-patent/pim-patent_zh_CN';
import trmtrainaddress_zh_CN from '@locale/lanres/entities/trm-train-address/trm-train-address_zh_CN';
import pimarchivesmanage_zh_CN from '@locale/lanres/entities/pim-archives-manage/pim-archives-manage_zh_CN';
import pcmprofile_zh_CN from '@locale/lanres/entities/pcm-profile/pcm-profile_zh_CN';
import trmtrainoutapply_zh_CN from '@locale/lanres/entities/trm-train-out-apply/trm-train-out-apply_zh_CN';
import pimarchivalcatalogue_zh_CN from '@locale/lanres/entities/pim-archival-catalogue/pim-archival-catalogue_zh_CN';
import parjxqdsz_zh_CN from '@locale/lanres/entities/par-jxqdsz/par-jxqdsz_zh_CN';
import vacsituationdetail_zh_CN from '@locale/lanres/entities/vac-situation-detail/vac-situation-detail_zh_CN';
import trmtraincourse_zh_CN from '@locale/lanres/entities/trm-train-course/trm-train-course_zh_CN';
import parkhfa_zh_CN from '@locale/lanres/entities/par-khfa/par-khfa_zh_CN';
import attendenceormorgscetor_zh_CN from '@locale/lanres/entities/att-endence-ormorgscetor/att-endence-ormorgscetor_zh_CN';
import trmdutycadres_zh_CN from '@locale/lanres/entities/trm-duty-cadres/trm-duty-cadres_zh_CN';
import vacoveralltion_zh_CN from '@locale/lanres/entities/vac-overalltion/vac-overalltion_zh_CN';
import partzgg_zh_CN from '@locale/lanres/entities/par-tzgg/par-tzgg_zh_CN';
import pcmydntmx_zh_CN from '@locale/lanres/entities/pcm-ydntmx/pcm-ydntmx_zh_CN';
import trmgradecadres_zh_CN from '@locale/lanres/entities/trm-gradecadres/trm-gradecadres_zh_CN';
import trmlgbdetail_zh_CN from '@locale/lanres/entities/trm-lgb-detail/trm-lgb-detail_zh_CN';
import ormrelation_zh_CN from '@locale/lanres/entities/orm-relation/orm-relation_zh_CN';
import trmtrainfacies_zh_CN from '@locale/lanres/entities/trm-train-facies/trm-train-facies_zh_CN';
import ormsignorg_zh_CN from '@locale/lanres/entities/orm-sign-org/orm-sign-org_zh_CN';
import pcmydgzmx_zh_CN from '@locale/lanres/entities/pcm-ydgzmx/pcm-ydgzmx_zh_CN';
import attendancesettings_zh_CN from '@locale/lanres/entities/att-endance-settings/att-endance-settings_zh_CN';
import salstddszn_zh_CN from '@locale/lanres/entities/sal-stddszn/sal-stddszn_zh_CN';
import ormdepestman_zh_CN from '@locale/lanres/entities/orm-dep-est-man/orm-dep-est-man_zh_CN';
import ormxmsfhz_zh_CN from '@locale/lanres/entities/orm-xmsfhz/orm-xmsfhz_zh_CN';
import ormduty_zh_CN from '@locale/lanres/entities/orm-duty/orm-duty_zh_CN';
import pcmmspjxx_zh_CN from '@locale/lanres/entities/pcm-mspjxx/pcm-mspjxx_zh_CN';
import codeitem_zh_CN from '@locale/lanres/entities/code-item/code-item_zh_CN';
import pimdaterule_zh_CN from '@locale/lanres/entities/pim-date-rule/pim-date-rule_zh_CN';
import ormqybzwh_zh_CN from '@locale/lanres/entities/orm-qybzwh/orm-qybzwh_zh_CN';
import vacholidayrules_zh_CN from '@locale/lanres/entities/vac-holiday-rules/vac-holiday-rules_zh_CN';
import pimworkflowref_zh_CN from '@locale/lanres/entities/pim-workflow-ref/pim-workflow-ref_zh_CN';
import parjxkhxhz_zh_CN from '@locale/lanres/entities/par-jxkhxhz/par-jxkhxhz_zh_CN';
import pcmxygzzjlmx_zh_CN from '@locale/lanres/entities/pcm-xygzzjlmx/pcm-xygzzjlmx_zh_CN';
import vacwfqj_zh_CN from '@locale/lanres/entities/vac-wfqj/vac-wfqj_zh_CN';
import pcmjxszzkhjgjl_zh_CN from '@locale/lanres/entities/pcm-jxszzkhjgjl/pcm-jxszzkhjgjl_zh_CN';
import salsource_zh_CN from '@locale/lanres/entities/sal-source/sal-source_zh_CN';
import pimeducation_zh_CN from '@locale/lanres/entities/pim-education/pim-education_zh_CN';
import ormqygl_zh_CN from '@locale/lanres/entities/orm-qygl/orm-qygl_zh_CN';
import pcmjxsygzzsqmx_zh_CN from '@locale/lanres/entities/pcm-jxsygzzsqmx/pcm-jxsygzzsqmx_zh_CN';
import pimprovince_zh_CN from '@locale/lanres/entities/pim-province/pim-province_zh_CN';
import pcmzsgl_zh_CN from '@locale/lanres/entities/pcm-zsgl/pcm-zsgl_zh_CN';
import ormpost_zh_CN from '@locale/lanres/entities/orm-post/orm-post_zh_CN';
import salplan_zh_CN from '@locale/lanres/entities/sal-plan/sal-plan_zh_CN';
import pcmzpmeglmx_zh_CN from '@locale/lanres/entities/pcm-zpmeglmx/pcm-zpmeglmx_zh_CN';
import parjxbzglmx_zh_CN from '@locale/lanres/entities/par-jxbzglmx/par-jxbzglmx_zh_CN';
import salstdgwgz_zh_CN from '@locale/lanres/entities/sal-stdgwgz/sal-stdgwgz_zh_CN';
import pimpersonabilitydetail_zh_CN from '@locale/lanres/entities/pim-person-ability-detail/pim-person-ability-detail_zh_CN';
import ormxmgl_zh_CN from '@locale/lanres/entities/orm-xmgl/orm-xmgl_zh_CN';
import attenmembwes_zh_CN from '@locale/lanres/entities/att-enmembwes/att-enmembwes_zh_CN';
import ormerporg_zh_CN from '@locale/lanres/entities/orm-erporg/orm-erporg_zh_CN';
import parbmfzjyx_zh_CN from '@locale/lanres/entities/par-bmfzjyx/par-bmfzjyx_zh_CN';
import pcmddsqd_zh_CN from '@locale/lanres/entities/pcm-ddsqd/pcm-ddsqd_zh_CN';
import pcmjxsgz_zh_CN from '@locale/lanres/entities/pcm-jxsgz/pcm-jxsgz_zh_CN';
import vacsituation_zh_CN from '@locale/lanres/entities/vac-situation/vac-situation_zh_CN';
import pcmjxsygzzjlmx_zh_CN from '@locale/lanres/entities/pcm-jxsygzzjlmx/pcm-jxsygzzjlmx_zh_CN';
import vacsystemapplication_zh_CN from '@locale/lanres/entities/vac-system-application/vac-system-application_zh_CN';
import trmzzobm_zh_CN from '@locale/lanres/entities/trm-zzobm/trm-zzobm_zh_CN';
import pcmydbxmx_zh_CN from '@locale/lanres/entities/pcm-ydbxmx/pcm-ydbxmx_zh_CN';
import pimpaper_zh_CN from '@locale/lanres/entities/pim-paper/pim-paper_zh_CN';
import attendrecorddetail_zh_CN from '@locale/lanres/entities/attend-record-detail/attend-record-detail_zh_CN';
import pcmxygzzjgwh_zh_CN from '@locale/lanres/entities/pcm-xygzzjgwh/pcm-xygzzjgwh_zh_CN';
import salparam_zh_CN from '@locale/lanres/entities/sal-param/sal-param_zh_CN';
import parjxlhmb_zh_CN from '@locale/lanres/entities/par-jxlhmb/par-jxlhmb_zh_CN';
import pcmxygzzsqmx_zh_CN from '@locale/lanres/entities/pcm-xygzzsqmx/pcm-xygzzsqmx_zh_CN';
import pimvocationalcatalog_zh_CN from '@locale/lanres/entities/pim-vocational-catalog/pim-vocational-catalog_zh_CN';
import pcmydtxmx_zh_CN from '@locale/lanres/entities/pcm-ydtxmx/pcm-ydtxmx_zh_CN';
import pcmpracticeexperience_zh_CN from '@locale/lanres/entities/pcm-practice-experience/pcm-practice-experience_zh_CN';
import parfzsmx_zh_CN from '@locale/lanres/entities/par-fzsmx/par-fzsmx_zh_CN';
import pimcity_zh_CN from '@locale/lanres/entities/pim-city/pim-city_zh_CN';
import pimvocational_zh_CN from '@locale/lanres/entities/pim-vocational/pim-vocational_zh_CN';
import pimfaminfo_zh_CN from '@locale/lanres/entities/pim-faminfo/pim-faminfo_zh_CN';
import pimenclosure_zh_CN from '@locale/lanres/entities/pim-enclosure/pim-enclosure_zh_CN';
import trmtrainresment_zh_CN from '@locale/lanres/entities/trm-train-resment/trm-train-resment_zh_CN';
import components_zh_CN from '@locale/lanres/components/components_zh_CN';
import codelist_zh_CN from '@locale/lanres/codelist/codelist_zh_CN';
import userCustom_zh_CN from '@locale/lanres/userCustom/userCustom_zh_CN';

export default {
    app: {
        commonWords:{
            error: '失败',
            success: '成功',
            ok: '确认',
            cancel: '取消',
        },
        gridpage: {
            choicecolumns: '选择列',
            refresh: '刷新',
            show: '显示',
            records: '条',
            totle: '共',
        },
        tabpage: {
            sureclosetip: {
                title: '关闭提醒',
                content: '表单数据已经修改，确定要关闭？',
            },
            closeall: '关闭所有',
            closeother: '关闭其他',
        },
        fileUpload: {
            caption: '上传',
        },
        searchButton: {
            search: '搜索',
            reset: '重置',
        },
        calendar:{
          today: '今天',
          month: '月',
          week: '周',
          day: '天',
          list: '列',
          dateSelectModalTitle: '选择要跳转的时间',
          gotoDate: '跳转',
        },
        // 非实体视图
        views: {
            index: {
                caption: '员工自助',
                title: '员工自助',
            },
        },
        utilview:{
            importview:"导入数据",
            warning:"警告",
            info:"请配置数据导入项" 
        },
        menus: {
            shenqing: {
                menuitem1: '我的申请',
                menuitem2: '休假申请',
                menuitem3: '出（国）境申请',
            },
            shp: {
                menuitem1: '我的信息',
                menuitem4: '个人信息',
                menuitem5: '教育信息',
                menuitem6: '语言能力',
                menuitem7: '家庭情况',
                menuitem8: '工作履历',
                menuitem9: '证书信息',
                menuitem10: '奖励惩罚',
                menuitem11: '军转干部',
                menuitem12: '学术成果',
                menuitem2: '我的查询',
                menuitem13: '分配信息',
                menuitem14: '档案信息',
                menuitem15: '绩效信息',
                menuitem16: '合同信息',
                menuitem3: '我的申请',
                menuitem17: '休假申请',
                menuitem18: '出（国）境申请',
                menuitem19: '附件预览',
            },
            zizhu: {
                menuitem4: '我的信息',
                menuitem5: '个人信息',
                menuitem6: '教育信息',
                menuitem7: '语言信息',
                menuitem8: '家庭信息',
                menuitem9: '工作履历',
                menuitem10: '证书信息',
                menuitem11: '职称信息',
                menuitem12: '奖励惩罚',
                menuitem13: '科研成果',
                menuitem14: '专利信息',
                menuitem15: '论文信息',
                menuitem16: '附件预览',
                menuitem17: '我的查询',
                menuitem19: '分配信息',
                menuitem20: '档案信息',
                menuitem21: '绩效信息',
                menuitem22: '合同信息',
                menuitem23: '军转干部',
                menuitem18: '我的申请',
                menuitem24: '休假申请',
                menuitem25: '出（国）境申请',
            },
            chaxun: {
                menuitem1: '我的查询',
                menuitem2: '分配信息',
                menuitem3: '档案信息',
                menuitem4: '绩效信息',
                menuitem5: '合同信息',
                menuitem6: '军转干部',
            },
            xinxi: {
                menuitem10: '我的信息',
                menuitem1: '个人信息',
                menuitem2: '教育信息',
                menuitem3: '语言能力',
                menuitem4: '家庭情况',
                menuitem5: '工作履历',
                menuitem11: '证书信息',
                menuitem12: '职称信息',
                menuitem7: '奖励惩罚',
                menuitem14: '科研成果',
                menuitem15: '专利信息',
                menuitem13: '论文信息',
                menuitem6: '附件预览',
            },
        },
    },
    entities: {
        sallog: sallog_zh_CN,
        trmdepart: trmdepart_zh_CN,
        trmtrainplanterm: trmtrainplanterm_zh_CN,
        salschemeitem: salschemeitem_zh_CN,
        trmlgbcost: trmlgbcost_zh_CN,
        pcmjxsygzzsq: pcmjxsygzzsq_zh_CN,
        pcmrecruitment: pcmrecruitment_zh_CN,
        pimcorrectionapply: pimcorrectionapply_zh_CN,
        ormorg: ormorg_zh_CN,
        pcmnotice: pcmnotice_zh_CN,
        salstdgwgzrate: salstdgwgzrate_zh_CN,
        pcmydmx: pcmydmx_zh_CN,
        pimqualtype: pimqualtype_zh_CN,
        pimmajorsetype: pimmajorsetype_zh_CN,
        pcmwzd0001: pcmwzd0001_zh_CN,
        pimtitle: pimtitle_zh_CN,
        parlhmbkhmx: parlhmbkhmx_zh_CN,
        attendencesetup: attendencesetup_zh_CN,
        pcmddsqdmx: pcmddsqdmx_zh_CN,
        pcmschooloffice: pcmschooloffice_zh_CN,
        pimtypecontract: pimtypecontract_zh_CN,
        trmteachercharge: trmteachercharge_zh_CN,
        salstdzc: salstdzc_zh_CN,
        pcmxygzzsq: pcmxygzzsq_zh_CN,
        vacworkday: vacworkday_zh_CN,
        attendencetype: attendencetype_zh_CN,
        pimoutput: pimoutput_zh_CN,
        parjxzgpc: parjxzgpc_zh_CN,
        vacsystemhistory: vacsystemhistory_zh_CN,
        attendancemreportmx: attendancemreportmx_zh_CN,
        trmteachertrain: trmteachertrain_zh_CN,
        parjxkhjcsz: parjxkhjcsz_zh_CN,
        trmtrainperson: trmtrainperson_zh_CN,
        pimarchivesloanandreturn: pimarchivesloanandreturn_zh_CN,
        pcmzjdyzw: pcmzjdyzw_zh_CN,
        pcmrcxl: pcmrcxl_zh_CN,
        pimbyzzjlmx: pimbyzzjlmx_zh_CN,
        ormbmgwbz: ormbmgwbz_zh_CN,
        pimcontract: pimcontract_zh_CN,
        saltype: saltype_zh_CN,
        pcmprofileinitnumber: pcmprofileinitnumber_zh_CN,
        pcmworkresume: pcmworkresume_zh_CN,
        trmtrainactment: trmtrainactment_zh_CN,
        pimsocialsecurity: pimsocialsecurity_zh_CN,
        pimworkhistory: pimworkhistory_zh_CN,
        ormorgdz: ormorgdz_zh_CN,
        parlhmbmx: parlhmbmx_zh_CN,
        pimpersonability: pimpersonability_zh_CN,
        trminvoice: trminvoice_zh_CN,
        ormtitle: ormtitle_zh_CN,
        vacleacetype: vacleacetype_zh_CN,
        salstdgw: salstdgw_zh_CN,
        pimarchiveschange: pimarchiveschange_zh_CN,
        pcmdetail: pcmdetail_zh_CN,
        pimexpaccount: pimexpaccount_zh_CN,
        pcmsyqgz: pcmsyqgz_zh_CN,
        vacholiday: vacholiday_zh_CN,
        salsalarydetail: salsalarydetail_zh_CN,
        parjxmtfk: parjxmtfk_zh_CN,
        attendancemreport: attendancemreport_zh_CN,
        trmtrainactapply: trmtrainactapply_zh_CN,
        pcmgxml: pcmgxml_zh_CN,
        pcmtxbz: pcmtxbz_zh_CN,
        ormxmxqhz: ormxmxqhz_zh_CN,
        pimqualmajor: pimqualmajor_zh_CN,
        pcmxygzzkhjgjl: pcmxygzzkhjgjl_zh_CN,
        parjxkhmbmx: parjxkhmbmx_zh_CN,
        vacleavemanage: vacleavemanage_zh_CN,
        pcmlog: pcmlog_zh_CN,
        pimvacation: pimvacation_zh_CN,
        pimexitandentry: pimexitandentry_zh_CN,
        trmplanformu: trmplanformu_zh_CN,
        trmtraindemand: trmtraindemand_zh_CN,
        pimpersonchange: pimpersonchange_zh_CN,
        pcmeducationexperience: pcmeducationexperience_zh_CN,
        trmtrainplan: trmtrainplan_zh_CN,
        pimachievements: pimachievements_zh_CN,
        pcmxkml: pcmxkml_zh_CN,
        pimlanguageability: pimlanguageability_zh_CN,
        pimperson: pimperson_zh_CN,
        trmtraffic: trmtraffic_zh_CN,
        pimexaminationresults: pimexaminationresults_zh_CN,
        pimarchives: pimarchives_zh_CN,
        vacleavesystemdtail: vacleavesystemdtail_zh_CN,
        parjxzgpcmx: parjxzgpcmx_zh_CN,
        ormrank: ormrank_zh_CN,
        vacleavedetail: vacleavedetail_zh_CN,
        trmtraindata: trmtraindata_zh_CN,
        pcmgbhmc: pcmgbhmc_zh_CN,
        parjgbmpjbz: parjgbmpjbz_zh_CN,
        parndlhmbkh: parndlhmbkh_zh_CN,
        pcmydjzmx: pcmydjzmx_zh_CN,
        pcmtestresult: pcmtestresult_zh_CN,
        ormorginfo: ormorginfo_zh_CN,
        vacinitnxj: vacinitnxj_zh_CN,
        salstdzx: salstdzx_zh_CN,
        pimworkflow: pimworkflow_zh_CN,
        salitem: salitem_zh_CN,
        trmagencyrecord: trmagencyrecord_zh_CN,
        ormbmkqdz: ormbmkqdz_zh_CN,
        pcmpcmsgqmgr: pcmpcmsgqmgr_zh_CN,
        trmdepartneeds: trmdepartneeds_zh_CN,
        salstdxmgz: salstdxmgz_zh_CN,
        pcmcsrcmspj: pcmcsrcmspj_zh_CN,
        parkhzcmx: parkhzcmx_zh_CN,
        salstdzyzg: salstdzyzg_zh_CN,
        pimarchivesrecord: pimarchivesrecord_zh_CN,
        pcmreason: pcmreason_zh_CN,
        ormorgsector: ormorgsector_zh_CN,
        trmdemdeftion: trmdemdeftion_zh_CN,
        parjxjg: parjxjg_zh_CN,
        salsubject: salsubject_zh_CN,
        pcmydlzmx: pcmydlzmx_zh_CN,
        pimtitlecatalogue: pimtitlecatalogue_zh_CN,
        parxmbpjbz: parxmbpjbz_zh_CN,
        vacleavesystem: vacleavesystem_zh_CN,
        pcmypzjkxx: pcmypzjkxx_zh_CN,
        salstdgl: salstdgl_zh_CN,
        pimarmycadres: pimarmycadres_zh_CN,
        pimbyygzzsq: pimbyygzzsq_zh_CN,
        trmtrainteacher: trmtrainteacher_zh_CN,
        pcmjxsbdjl: pcmjxsbdjl_zh_CN,
        trmtrainbuapply: trmtrainbuapply_zh_CN,
        codelist1: codelist1_zh_CN,
        trmtrainagency: trmtrainagency_zh_CN,
        pimresearchfindings: pimresearchfindings_zh_CN,
        salscheme: salscheme_zh_CN,
        pimlwgz: pimlwgz_zh_CN,
        pimcontracttype: pimcontracttype_zh_CN,
        salsalary: salsalary_zh_CN,
        pcmnorecyclinglibrary: pcmnorecyclinglibrary_zh_CN,
        attendancedate: attendancedate_zh_CN,
        pimnationmgr: pimnationmgr_zh_CN,
        ormxmrypzxq: ormxmrypzxq_zh_CN,
        pcmydjpmx: pcmydjpmx_zh_CN,
        pimdistirbution: pimdistirbution_zh_CN,
        pimlabourcampany: pimlabourcampany_zh_CN,
        pcmydjdmx: pcmydjdmx_zh_CN,
        pcmtdzwxx: pcmtdzwxx_zh_CN,
        pimcontractsignorg: pimcontractsignorg_zh_CN,
        parjxkhmb: parjxkhmb_zh_CN,
        salpersonstd: salpersonstd_zh_CN,
        trmtraindemandsum: trmtraindemandsum_zh_CN,
        salsalarybill: salsalarybill_zh_CN,
        parznbmmx: parznbmmx_zh_CN,
        trmemployeeretion: trmemployeeretion_zh_CN,
        parjxndkhjg: parjxndkhjg_zh_CN,
        trmstaff: trmstaff_zh_CN,
        ormxmbmx: ormxmbmx_zh_CN,
        parldkhqz: parldkhqz_zh_CN,
        pimquestions: pimquestions_zh_CN,
        salruledetail: salruledetail_zh_CN,
        salpersonstddetail: salpersonstddetail_zh_CN,
        trmstay: trmstay_zh_CN,
        trmcoursesystem: trmcoursesystem_zh_CN,
        attensummary: attensummary_zh_CN,
        salrule: salrule_zh_CN,
        ormxmrlcb: ormxmrlcb_zh_CN,
        personstatemgr: personstatemgr_zh_CN,
        ormpostdetails: ormpostdetails_zh_CN,
        pimstafftype: pimstafftype_zh_CN,
        pcmsgqmgr: pcmsgqmgr_zh_CN,
        salitemsub: salitemsub_zh_CN,
        pimgwtype: pimgwtype_zh_CN,
        ormpostlib: ormpostlib_zh_CN,
        pimarchivescenter: pimarchivescenter_zh_CN,
        parjxbzgl: parjxbzgl_zh_CN,
        vacusenxjmx: vacusenxjmx_zh_CN,
        vacsynjcx: vacsynjcx_zh_CN,
        pcmzpmegl: pcmzpmegl_zh_CN,
        attensummarymx: attensummarymx_zh_CN,
        trmhmatser: trmhmatser_zh_CN,
        pcmtxfpsq: pcmtxfpsq_zh_CN,
        ormzwdqgz: ormzwdqgz_zh_CN,
        pcmyddgmx: pcmyddgmx_zh_CN,
        ormxmbq: ormxmbq_zh_CN,
        attendancerecordtemp: attendancerecordtemp_zh_CN,
        pcmgxxktemp: pcmgxxktemp_zh_CN,
        pcmrzgl: pcmrzgl_zh_CN,
        trmtrainfillin: trmtrainfillin_zh_CN,
        salstdxmgzrate: salstdxmgzrate_zh_CN,
        vacdayoff: vacdayoff_zh_CN,
        trmstaffnodes: trmstaffnodes_zh_CN,
        pcmawardswons: pcmawardswons_zh_CN,
        trmcouarrange: trmcouarrange_zh_CN,
        pimrewardpunishment: pimrewardpunishment_zh_CN,
        salstdzj: salstdzj_zh_CN,
        attendancerecord: attendancerecord_zh_CN,
        pimpatent: pimpatent_zh_CN,
        trmtrainaddress: trmtrainaddress_zh_CN,
        pimarchivesmanage: pimarchivesmanage_zh_CN,
        pcmprofile: pcmprofile_zh_CN,
        trmtrainoutapply: trmtrainoutapply_zh_CN,
        pimarchivalcatalogue: pimarchivalcatalogue_zh_CN,
        parjxqdsz: parjxqdsz_zh_CN,
        vacsituationdetail: vacsituationdetail_zh_CN,
        trmtraincourse: trmtraincourse_zh_CN,
        parkhfa: parkhfa_zh_CN,
        attendenceormorgscetor: attendenceormorgscetor_zh_CN,
        trmdutycadres: trmdutycadres_zh_CN,
        vacoveralltion: vacoveralltion_zh_CN,
        partzgg: partzgg_zh_CN,
        pcmydntmx: pcmydntmx_zh_CN,
        trmgradecadres: trmgradecadres_zh_CN,
        trmlgbdetail: trmlgbdetail_zh_CN,
        ormrelation: ormrelation_zh_CN,
        trmtrainfacies: trmtrainfacies_zh_CN,
        ormsignorg: ormsignorg_zh_CN,
        pcmydgzmx: pcmydgzmx_zh_CN,
        attendancesettings: attendancesettings_zh_CN,
        salstddszn: salstddszn_zh_CN,
        ormdepestman: ormdepestman_zh_CN,
        ormxmsfhz: ormxmsfhz_zh_CN,
        ormduty: ormduty_zh_CN,
        pcmmspjxx: pcmmspjxx_zh_CN,
        codeitem: codeitem_zh_CN,
        pimdaterule: pimdaterule_zh_CN,
        ormqybzwh: ormqybzwh_zh_CN,
        vacholidayrules: vacholidayrules_zh_CN,
        pimworkflowref: pimworkflowref_zh_CN,
        parjxkhxhz: parjxkhxhz_zh_CN,
        pcmxygzzjlmx: pcmxygzzjlmx_zh_CN,
        vacwfqj: vacwfqj_zh_CN,
        pcmjxszzkhjgjl: pcmjxszzkhjgjl_zh_CN,
        salsource: salsource_zh_CN,
        pimeducation: pimeducation_zh_CN,
        ormqygl: ormqygl_zh_CN,
        pcmjxsygzzsqmx: pcmjxsygzzsqmx_zh_CN,
        pimprovince: pimprovince_zh_CN,
        pcmzsgl: pcmzsgl_zh_CN,
        ormpost: ormpost_zh_CN,
        salplan: salplan_zh_CN,
        pcmzpmeglmx: pcmzpmeglmx_zh_CN,
        parjxbzglmx: parjxbzglmx_zh_CN,
        salstdgwgz: salstdgwgz_zh_CN,
        pimpersonabilitydetail: pimpersonabilitydetail_zh_CN,
        ormxmgl: ormxmgl_zh_CN,
        attenmembwes: attenmembwes_zh_CN,
        ormerporg: ormerporg_zh_CN,
        parbmfzjyx: parbmfzjyx_zh_CN,
        pcmddsqd: pcmddsqd_zh_CN,
        pcmjxsgz: pcmjxsgz_zh_CN,
        vacsituation: vacsituation_zh_CN,
        pcmjxsygzzjlmx: pcmjxsygzzjlmx_zh_CN,
        vacsystemapplication: vacsystemapplication_zh_CN,
        trmzzobm: trmzzobm_zh_CN,
        pcmydbxmx: pcmydbxmx_zh_CN,
        pimpaper: pimpaper_zh_CN,
        attendrecorddetail: attendrecorddetail_zh_CN,
        pcmxygzzjgwh: pcmxygzzjgwh_zh_CN,
        salparam: salparam_zh_CN,
        parjxlhmb: parjxlhmb_zh_CN,
        pcmxygzzsqmx: pcmxygzzsqmx_zh_CN,
        pimvocationalcatalog: pimvocationalcatalog_zh_CN,
        pcmydtxmx: pcmydtxmx_zh_CN,
        pcmpracticeexperience: pcmpracticeexperience_zh_CN,
        parfzsmx: parfzsmx_zh_CN,
        pimcity: pimcity_zh_CN,
        pimvocational: pimvocational_zh_CN,
        pimfaminfo: pimfaminfo_zh_CN,
        pimenclosure: pimenclosure_zh_CN,
        trmtrainresment: trmtrainresment_zh_CN,
    },
    components: components_zh_CN,
    codelist: codelist_zh_CN,
    userCustom: userCustom_zh_CN,
};