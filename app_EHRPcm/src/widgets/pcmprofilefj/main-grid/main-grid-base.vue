...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pcmprofilefj.main_grid.columns.fj')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.fj}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fjname')">
                <el-table-column show-overflow-tooltip :prop="'fjname'" :label="$t('entities.pcmprofilefj.main_grid.columns.fjname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.fjname}}</span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pcmprofilefj.main_grid.columns.fj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fjname',
            label: '附件名称',
            langtag: 'entities.pcmprofilefj.main_grid.columns.fjname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者附件标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者附件标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILEFJProfileFJGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILEFJProfileFJGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmprofilefjname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmprofilefj: keys.join(';') }),Object.assign({ pcmprofilefj: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmprofilefj,$event.pcmprofilefj);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmprofilefj_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmprofilefj_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMPROFILEFJProfileFJGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMPROFILEFJProfileFJGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pcmprofilefj){
                    Object.assign(this.context,{pcmprofilefj:item.pcmprofilefj})
                }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmprofilefj,args.row.pcmprofilefj);
        });
