...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pcmzpmeglmxname1')">
                <el-table-column show-overflow-tooltip :prop="'pcmzpmeglmxname1'" :label="$t('entities.pcmzpmeglmx.main_grid.columns.pcmzpmeglmxname1')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.pcmzpmeglmxname1}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pfrq')">
                <el-table-column show-overflow-tooltip :prop="'pfrq'" :label="$t('entities.pcmzpmeglmx.main_grid.columns.pfrq')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-format-data format="YYYY-MM-DD" :data="row.pfrq"></app-format-data>
                    </template>
                </el-table-column>
            </template>
...
import PCMZPMEGLMXService from '@/service/pcmzpmeglmx/pcmzpmeglmx-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {PCMZPMEGLMXService}
     * @memberof Main
     */
    public appEntityService: PCMZPMEGLMXService = new PCMZPMEGLMXService({ $store: this.$store });
     ...
     * @memberof Main
     */
    public minorSortPSDEF: string = 'pfrq';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pcmzpmeglmxname1',
            label: '招聘名额管理明细名称',
            langtag: 'entities.pcmzpmeglmx.main_grid.columns.pcmzpmeglmxname1',
            show: true,
            util: 'STAR'
        },
        {
            name: 'pfrq',
            label: '批复日期',
            langtag: 'entities.pcmzpmeglmx.main_grid.columns.pfrq',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '招聘名额管理明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '招聘名额管理明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLMXZPMEGLMXGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLMXZPMEGLMXGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmzpmeglmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmzpmeglmx: keys.join(';') }),Object.assign({ pcmzpmeglmx: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmzpmeglmx,$event.pcmzpmeglmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmzpmeglmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmzpmeglmx_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLMXZPMEGLMXGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLMXZPMEGLMXGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pcmzpmeglmx){
                    Object.assign(this.context,{pcmzpmeglmx:item.pcmzpmeglmx})
                }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmzpmeglmx,args.row.pcmzpmeglmx);
        });
