                ...
                <el-table-column show-overflow-tooltip :prop="'xkml'" :label="$t('entities.pcmxkml.main_grid.columns.xkml')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PIMCL_XKML' codelistType='DYNAMIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.xkml" tag='PIMCL_XKML' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmxkml: keys.join(';') }),Object.assign({ pcmxkml: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
