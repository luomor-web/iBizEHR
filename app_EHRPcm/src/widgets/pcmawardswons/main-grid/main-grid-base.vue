                ...
                <el-table-column show-overflow-tooltip :prop="'awardlevel'" :label="$t('entities.pcmawardswons.main_grid.columns.awardlevel')" :width="250" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PCMCL_HJDJ' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.awardlevel" tag='PCMCL_HJDJ' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: PCMAWARDSWONSService = new PCMAWARDSWONSService({ $store: this.$store });
    ...
    */
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmawardswons: keys.join(';') }),Object.assign({ pcmawardswons: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
