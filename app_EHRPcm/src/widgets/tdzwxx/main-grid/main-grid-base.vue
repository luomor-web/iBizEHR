...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('initapplydate')">
                <el-table-column show-overflow-tooltip :prop="'initapplydate'" :label="$t('entities.tdzwxx.main_grid.columns.initapplydate')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                    <date-picker type="date" :transfer="true" format="yyyy-MM-dd" placeholder="请选择时间..." :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" :value="row[column.property]" style="width:100%;" @on-change="(val1, val2) => { row[column.property] = val1; gridEditItemChange(row, column.property, val1)}"></date-picker>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            ...
            <template v-if="getColumnState('department')">
                <el-table-column show-overflow-tooltip :prop="'department'" :label="$t('entities.tdzwxx.main_grid.columns.department')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('phaseinfo')">
                <el-table-column show-overflow-tooltip :prop="'phaseinfo'" :label="$t('entities.tdzwxx.main_grid.columns.phaseinfo')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('statusinfo')">
                <el-table-column show-overflow-tooltip :prop="'statusinfo'" :label="$t('entities.tdzwxx.main_grid.columns.statusinfo')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: TDZWXXService = new TDZWXXService({ $store: this.$store });
     ...
     * @memberof Main
     */
    protected minorSortPSDEF: string = 'initapplydate';
    ...
    */
    ...
    protected allColumns: any[] = [
        ...
        },
        {
            name: 'tdzwxxname',
            label: '职位名称（*）',
            langtag: 'entities.tdzwxx.main_grid.columns.tdzwxxname',
            show: true,
            util: 'PX'
        },
        {
            name: 'initapplydate',
            label: '最初投递时间（*）',
            langtag: 'entities.tdzwxx.main_grid.columns.initapplydate',
            show: true,
            util: 'PX'
        },
        {
            name: 'department',
            label: '投递组织（*）',
            langtag: 'entities.tdzwxx.main_grid.columns.department',
            show: true,
            util: 'PX'
        },
        {
            name: 'phaseinfo',
            label: '所在阶段信息（*）',
            langtag: 'entities.tdzwxx.main_grid.columns.phaseinfo',
            show: true,
            util: 'PX'
        },
        {
            name: 'statusinfo',
            label: '所在状态信息（*）',
            langtag: 'entities.tdzwxx.main_grid.columns.statusinfo',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ tdzwxx: keys.join(';') }),Object.assign({ tdzwxx: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected save(args: any[], params?: any, $event?: any, xData?: any): void {
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                Object.assign(item,{viewparams:this.viewparams});
                if(item.tdzwxx){
                    Object.assign(this.context,{tdzwxx:item.tdzwxx})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.tdzwxx,args.row.tdzwxx);
        });
