...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.pcmgxml.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='PIMCL_ND' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.pcmgxml.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.pcmgxml.main_grid.columns.updatedate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.pcmgxml.main_grid.columns.updateman',
            show: true,
            util: 'px'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.pcmgxml.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLCurNDGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLCurNDGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmgxml: keys.join(';') }),Object.assign({ pcmgxml: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMGXMLCurNDGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMGXMLCurNDGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.pcmgxml){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLCurNDGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
