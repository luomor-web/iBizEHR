    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('yglx')">
                <el-table-column show-overflow-tooltip :prop="'yglx'" :label="$t('entities.pcmprofileszyjtj.ypzsy_grid.columns.yglx')" :width="110"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmprofileszyjtj.ypzsy_grid.columns.yglx')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pcmprofileszyjtjname')">
                <el-table-column show-overflow-tooltip :prop="'pcmprofileszyjtjname'" :label="$t('entities.pcmprofileszyjtj.ypzsy_grid.columns.pcmprofileszyjtjname')" :min-width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmprofileszyjtj.ypzsy_grid.columns.pcmprofileszyjtjname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='pcmprofileszyjtjname' editorType="TEXTBOX" :value="row.pcmprofileszyjtjname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
import PCMPROFILESZYJTJService from '@/service/pcmprofileszyjtj/pcmprofileszyjtj-service';
import YPZSYService from './ypzsy-grid-service';
...
export default class YPZSYBase extends Vue implements ControlInterface {
     ...
     * @memberof YPZSY
     */
    public service: YPZSYService = new YPZSYService({ $store: this.$store });

    /**
     * 实体服务对象
     *
     * @type {PCMPROFILESZYJTJService}
     * @memberof YPZSY
     */
    public appEntityService: PCMPROFILESZYJTJService = new PCMPROFILESZYJTJService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'yglx',
            label: '申报类型',
            langtag: 'entities.pcmprofileszyjtj.ypzsy_grid.columns.yglx',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '应聘者社招引进条件',
            langtag: 'entities.pcmprofileszyjtj.ypzsy_grid.columns.pcmprofileszyjtjname',
            show: true,
            util: 'STAR'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          pcmprofileszyjtjname: new FormItemModel(),
     ...
     */
    public rules: any = {
        pcmprofileszyjtjname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者社招引进条件 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者社招引进条件 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者社招引进条件标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者社招引进条件标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILESZYJTJPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILESZYJTJPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmprofileszyjtjname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmprofileszyjtj: _keys }),Object.assign({ pcmprofileszyjtj: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmprofileszyjtj,$event.pcmprofileszyjtj);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmprofileszyjtj_ypzsy_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmprofileszyjtj_ypzsy_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMPROFILESZYJTJPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMPROFILESZYJTJPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmprofileszyjtj){
                            Object.assign(this.context,{pcmprofileszyjtj:item.pcmprofileszyjtj});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmprofileszyjtj,args.row.pcmprofileszyjtj);
        });
