...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('cadredescription')">
                <el-table-column show-overflow-tooltip :prop="'cadredescription'" :label="$t('entities.pcmschooloffice.main_grid.columns.cadredescription')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: PCMSCHOOLOFFICEService = new PCMSCHOOLOFFICEService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZWGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZWGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmschooloffice: keys.join(';') }),Object.assign({ pcmschooloffice: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZWGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZWGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZWGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
