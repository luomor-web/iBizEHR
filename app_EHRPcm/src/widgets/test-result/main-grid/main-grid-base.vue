...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('psychologystatus')">
                <el-table-column show-overflow-tooltip :prop="'psychologystatus'" :label="$t('entities.testresult.main_grid.columns.psychologystatus')" :width="150" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('abilitygrade')">
                <el-table-column show-overflow-tooltip :prop="'abilitygrade'" :label="$t('entities.testresult.main_grid.columns.abilitygrade')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('jobfitdegree')">
                <el-table-column show-overflow-tooltip :prop="'jobfitdegree'" :label="$t('entities.testresult.main_grid.columns.jobfitdegree')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: TestResultService = new TestResultService({ $store: this.$store });
    ...
    */
    ...
    protected allColumns: any[] = [
        {
            ...
            label: '心理健康（*）',
            langtag: 'entities.testresult.main_grid.columns.psychologystatus',
            show: true,
            util: 'PX'
        },
        {
            name: 'abilitygrade',
            label: '能力（*）',
            langtag: 'entities.testresult.main_grid.columns.abilitygrade',
            show: true,
            util: 'PX'
        },
        {
            name: 'jobfitdegree',
            label: '胜任力（*）',
            langtag: 'entities.testresult.main_grid.columns.jobfitdegree',
            show: true,
            util: 'PX'
        },
    ]
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJGGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJGGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ testresult: keys.join(';') }),Object.assign({ testresult: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TestResultCPJGGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TestResultCPJGGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.testresult){
                    Object.assign(this.context,{testresult:item.testresult})
                }
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJGGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.testresult,args.row.testresult);
        });
