...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.wzd0001.ypzbd_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.profilelist.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='profilelist' :itemRules="this.rules.profilelist" class='' :caption="$t('entities.wzd0001.ypzbd_form.details.profilelist')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.profilelist.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.peoplenum.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='peoplenum' :itemRules="this.rules.peoplenum" class='' :caption="$t('entities.wzd0001.ypzbd_form.details.peoplenum')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.peoplenum.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.bdsj.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='bdsj' :itemRules="this.rules.bdsj" class='' :caption="$t('entities.wzd0001.ypzbd_form.details.bdsj')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.bdsj.error" :isEmptyCaption="false" labelPos="LEFT">
    <date-picker type="date" :transfer="true" format="yyyy-MM-dd" placeholder="请选择时间..." :value="data.bdsj" :disabled="detailsModel.bdsj.disabled" style="min-width: 150px; width:100px;width:100%;" @on-change="(val1, val2) => { this.data.bdsj = val1 }"></date-picker>
</app-form-item>
...
import YPZBDService from './ypzbd-form-service';
...
export default class YPZBDBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        bdsj: [
            { type: 'string', message: '报到时间 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '报到时间 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '报到时间 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '报到时间 值不能为空', trigger: 'blur' },
        ],
        wzd0001id: [
            { type: 'string', message: '向导实体1标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '向导实体1标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        group1: new FormGroupPanelModel({ caption: '选择人员及时间', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.wzd0001.ypzbd_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        bdsj: new FormItemModel({ caption: '报到时间', detailType: 'FORMITEM', name: 'bdsj', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.bdsj')
    onBdsjChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'bdsj', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'WZD0001YPZBDOptionView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'WZD0001YPZBDOptionView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'WZD0001YPZBDOptionView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'WZD0001YPZBDOptionView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
