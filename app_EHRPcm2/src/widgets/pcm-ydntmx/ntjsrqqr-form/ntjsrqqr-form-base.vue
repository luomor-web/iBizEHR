<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pcmydntmx_ntjsrqqr' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pcmydntmx.ntjsrqqr_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.jsrq.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='jsrq' :itemRules="this.rules.jsrq" class='' :caption="$t('entities.pcmydntmx.ntjsrqqr_form.details.jsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
import NTJSRQQRService from './ntjsrqqr-form-service';
...
export default class NTJSRQQRBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof NTJSRQQRBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {NTJSRQQRService}
     * @memberof NTJSRQQRBase
     */
    public service: NTJSRQQRService = new NTJSRQQRService({ $store: this.$store });
    ...
    public rules: any = {
        ...
        jsrq: [
            ...
            { required: true, type: 'string', message: '内退结束日期 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '内退结束日期 值不能为空', trigger: 'blur' },
        ],
        pcmydntmxid: [
            { type: 'string', message: '异动内退明细标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '异动内退明细标识 值必须为字符串类型', trigger: 'blur' },
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '异动内退明细基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.pcmydntmx.ntjsrqqr_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        jsrq: new FormItemModel({ caption: '内退结束日期', detailType: 'FORMITEM', name: 'jsrq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     *
     * @param {*} [arg={}]
     * @memberof @memberof NTJSRQQRBase
     */
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pcmydntmx_ntjsrqqr',popTitle:'内退结束日期确认'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdntmxNTJSRQQREditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdntmxNTJSRQQREditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PcmYdntmxNTJSRQQREditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PcmYdntmxNTJSRQQREditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
