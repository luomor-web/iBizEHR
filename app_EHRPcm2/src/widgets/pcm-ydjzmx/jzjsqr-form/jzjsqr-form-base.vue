<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pcmydjzmx_jzjsqr' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pcmydjzmx.jzjsqr_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.jsrq.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='jsrq' :itemRules="this.rules.jsrq" class='' :caption="$t('entities.pcmydjzmx.jzjsqr_form.details.jsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
import JZJSQRService from './jzjsqr-form-service';
...
export default class JZJSQRBase extends Vue implements ControlInterface {
     ...
     * @memberof JZJSQRBase
     */
    public service: JZJSQRService = new JZJSQRService({ $store: this.$store });
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '异动兼职明细基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.pcmydjzmx.jzjsqr_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        ksrq: new FormItemModel({ caption: '兼职开始日期', detailType: 'FORMITEM', name: 'ksrq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
...
, 
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pcmydjzmx_jzjsqr',popTitle:'兼职结束时间确定'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjzmxJSRQQREditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjzmxJSRQQREditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PcmYdjzmxJSRQQREditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PcmYdjzmxJSRQQREditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
