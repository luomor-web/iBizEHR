<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='pcmydjdmx_jdjsqr' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.pcmydjdmx.jdjsqr_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.jdjsrq.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='jdjsrq' :itemRules="this.rules.jdjsrq" class='' :caption="$t('entities.pcmydjdmx.jdjsqr_form.details.jdjsrq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.jdjsrq.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class JDJSQRBase extends Vue implements ControlInterface {
...
, 
        jdksrq: new FormItemModel({ caption: '借调开始日期', detailType: 'FORMITEM', name: 'jdksrq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 0 })
, 
...
, 
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'pcmydjdmx_jdjsqr',popTitle:'借调结束确认表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDQROptionView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDQROptionView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDQROptionView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDQROptionView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
